...
// The route fields are spelled once, beside InputDeviceIdentity itself, because the shared
// audio-config store persists the same four names and one physical route has to read the same in
// either file.
    ...
    for (const PlayerInputConfig& player : config.players)
    {
        array.append(
            common::core::Json::makeObject({
                ...
                {g_identity_backend_name_property,
        ...
        config.players.push_back(
            PlayerInputConfig{
                ...
                .route = common::audio::InputDeviceIdentity{
                    .backend_name = common::core::Json::readOptionalString(
                        entry, g_identity_backend_name_property),
                    .input_device_name = common::core::Json::readOptionalString(
...
GameSettings::GameSettings(const std::filesystem::path& settings_file)
    : m_properties{
          ...
          common::core::settingsFileOptions(common::core::gameApplicationName())
      }
{}
