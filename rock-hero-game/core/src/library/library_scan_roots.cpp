    ...
    std::span<const std::filesystem::path> custom_roots)
{
    ...
    // `/` and `\` as equal, and the normalized path key stays lossless (a narrowed string would
    // drop characters outside the Windows code page). The original path is preserved in the output
    // for the lister. Case differences on Windows are not folded here — plan 10's package identity
    // hash is the durable cross-spelling dedup.
