    ...
    const LibraryIndex& previous_index, const std::span<const PackageFileFacts> current_files)
{
    ...
    // benign re-describe, never a crash).
    // docs/plans/roadmap/10-format-versioning-and-chart-identity.md's package identity hash is the
    // designed real fix for stable identity across path spellings.
