...
Tracktion-backed implementation is intentionally minimal and creates only one backend track.
Unsupported track and clip creates fail explicitly rather than pretending that independent backend
playback exists.
*/
class IEdit
{
public:
    ...
    /*!
    \brief Creates a backend track mapped to a framework-free Session track id.
    ...
    On success, implementations return the framework-free track data that Session should attach to
    the allocated id.
    ...
    \return Accepted track data when the backend track was created and mapped; std::nullopt
    otherwise.
    */
    [[nodiscard]] virtual std::optional<core::TrackData> createTrack(
        core::TrackId track_id, const std::string& name) = 0;

    /*!
    \brief Creates an audio clip from an asset and returns the backend-accepted value.
    ...
    data separate from session-owned identity.
    ...
    \return Accepted clip data when the backend created it; std::nullopt otherwise.
    \note The track id must already have been mapped through createTrack().
    */
    [[nodiscard]] virtual std::optional<core::AudioClipData> createAudioClip(
