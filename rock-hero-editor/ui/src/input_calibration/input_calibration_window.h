...
public:
    ...
    \param read_only_game_reflection True when the editor sources the game's audio configuration, so
           ...
           and gain slider stay visible but disabled (grayed out), each carrying a "Derived from
           game settings" tooltip (plan 48 P2). False runs the full
           editable strum-to-calibrate flow.
    */
