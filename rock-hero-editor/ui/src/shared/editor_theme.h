...
struct EditorTheme
{
    ...
    */
    ...
    /*!
    \brief Editor furniture drawn OVER the tab lane's notation: the armed caret's square.

    Translucent white so the notation reads through it and it stays visible over every string
    color. A distinct ink from the notation's own, because it says something the notation never
    does — where the next edit lands — and the square shape keeps it apart from every circular
    note mark it may sit over.
    */
...
/*!
\brief How far a quieted editor mark falls toward the ground it is painted on.
...
mark happens to be drawn over, which is why the DISTANCE is the only number worth naming. A fixed
ground constant could not stand in for it: it is right only on a surface that happens to be exactly
that color, and the timeline paints three different row bands under one grid (the tone row's
0xff1a1e25 is lighter and bluer than the lane's near-black, so a mark pre-mixed toward that
near-black lands on top of the tone band and vanishes there). \ref quieted needs no ground constant
at all, because the compositor supplies the real one.
...
darkened control reads as unavailable, which is a different claim from "not binding right now".
State on borrowed chrome is said with a mark instead (the grid readout's snap-off strike).
