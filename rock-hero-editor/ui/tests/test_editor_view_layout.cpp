...
TEST_CASE("EditorView lays out toolbar below the menu bar", "[ui][editor-view]")
{
    ...
    CHECK(timeline_ruler.getBounds() == juce::Rectangle<int>{0, 0, 484, 58});
    CHECK(viewport.getBounds() == juce::Rectangle<int>{0, 58, 484, 22});
...
TEST_CASE("EditorView lays out the default track viewport", "[ui][editor-view]")
{
    ...
    CHECK(timeline_ruler.getBounds() == juce::Rectangle<int>{0, 0, 1264, 58});
    CHECK(viewport.getBounds() == juce::Rectangle<int>{0, 58, 1264, 414});
