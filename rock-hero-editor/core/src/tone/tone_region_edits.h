...
struct [[nodiscard]] ToneRegionToneEdit final : IEdit
{
    ...
    */
    ToneRegionToneEdit(
        ...
        std::string after_name_value, std::optional<common::core::Tone> added_tone_value,
        std::optional<common::core::Tone> removed_tone_value)
        : region_id(std::move(region_id_value))
        , before_ref(std::move(before_ref_value))
        , after_ref(std::move(after_ref_value))
        , after_name(std::move(after_name_value))
        , added_tone(std::move(added_tone_value))
        , removed_tone(std::move(removed_tone_value))
    {}
    ...
    */
    ...
    /*! \brief Catalog entry this retone minted, absent when it pointed at an existing tone. */
    std::optional<common::core::Tone> added_tone;

    /*! \brief Catalog entry pruned because this retone took its last reference, if any. */
