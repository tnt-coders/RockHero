...
# reference by name (the NAMING FIREWALL constraint restated in every docs/plans/*.md
...
foreach ($text in $texts)
{
        ...
        $text.IndexOf($forbidden, [System.StringComparison]::OrdinalIgnoreCase) -ge 0)
    {
        [Console]::Error.WriteLine(
            ...
            "section of every docs/plans/*.md plan): that name must never appear in any file, " +
