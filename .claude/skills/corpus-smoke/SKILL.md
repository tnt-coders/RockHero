...
- Never wire this check into CI or any committed test baseline. CI fixtures are self-authored,
  freely-licensed content only (see docs/plans/roadmap/23-detection-verification-harness.md,
  "Corpus firewall").
...
environment variable (the convention docs/plans/roadmap/11 and 23 standardize). If it is not set and the
...
- docs/plans/roadmap/23-detection-verification-harness.md Phase 7: Catch2 hidden-tag `[.local-corpus]`
  soak tests reading `ROCKHERO_CORPUS_DIR`, run by invoking the built test executable directly
  with the hidden tag.
- docs/plans/roadmap/11-derived-difficulty-calculator.md has an equivalent hidden-tag corpus sweep.
