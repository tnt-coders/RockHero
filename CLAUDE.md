...
| `-Wmissing-designated-field-initializers` | omitted aggregate fields accepted | GCC, Clang — and it fires in files the change never touched: giving a struct a NEW non-DMI field breaks every designated initializer of that struct anywhere in the tree, exactly as a new enumerator breaks every switch. Sweep by the SIBLING FIELD (`rg '\.first_field = '`), never by the type name — an initializer is usually spelled `Type name{.field = ...}`, which an `rg 'Type\{'` search misses entirely |
| `-Wshadow` against an *inherited* base member | silent | GCC |
| `-Wshadow-all` on a lambda **init-capture** that reuses the name it moves from | silent | Clang — JUCE's Clang flag set carries `-Wshadow-all`, and its `-Wshadow-uncaptured-local` fires on `[name = std::move(name)]`: the capture shadows the enclosing local it is initialized from. Rename the capture `owned_<name>` and read that name in the body, the spelling every other init-capture in the tree uses |
| `-Wshadow` inside a **Catch2 test file** — usually a lambda parameter named like a local of the enclosing test, as `[](const HighwayBeatViewState& beat)` below a `const double beat` | silent, and the per-file sweep below is silent too unless given two defines: against the MSVC compile database Catch2 sees `_MSC_VER` and `__clang__` together, leaves its warning push/pop macros empty, and the `clang diagnostic ignored "-Wshadow"` pragma its `TEST_CASE` expands to then leaks to the end of the file (taking `-Wunused-variable` and `-Wparentheses` with it) | GCC (`-Wshadow`) and macOS Clang (`-Wshadow-uncaptured-local`). Rename the newer of the two names |
| `-Wunused-function` on internal-linkage helpers | unused anonymous-namespace functions accepted | GCC, Clang |
| `-Wmissing-prototypes` on a file-scope helper written **outside** the anonymous namespace | silent — MSVC has no equivalent at `/W4`. The per-file clang-tidy sweep below reproduces it only when the flag itself is passed (`--extra-arg=/clang:-Wmissing-prototypes`): naming `clang-diagnostic-missing-prototypes` alone enables nothing | **macOS only** — both the Clang build and CI lint, which reports it as `clang-diagnostic-missing-prototypes`; the Linux GCC flag set does not carry `-Wmissing-declarations`, so Linux stays green on it. The mirror image of `-Wunused-function` above: that one fires when an internal-linkage helper loses its callers, this one when a helper never got internal linkage at all. A test fixture written next to the `TEST_CASE` that uses it, instead of in the file's `namespace { }` block, is the usual cause. Move it in beside its sibling fixtures — do not reach for `static`, which would make one helper's linkage differ from every other in the file |
| `-Wunused-lambda-capture` | not implemented | GCC, Clang — a name the lambda reaches only *through another lambda* it captured, and a `const` integral its body reads as a constant expression, both need no capture; naming them anyway is an error. Drop the capture |
| `-Wsign-conversion` on a **compound assignment** | silent | **GCC only** — `size_t_count += cond ? 1 : 0` converts an `int` into the unsigned target. Clang proves both arms non-negative and stays quiet, and GCC itself stays quiet for the same expression in an *initializer* (`size_t n = v.size() + (cond ? 0 : 1)`), so only the `+=` form fails and only on Linux. Write the branch instead: `if (cond) { ++count; }` |
| `-Wswitch-enum` on a switch a **new enumerator** just made non-exhaustive | silent — MSVC's C4061 is off at `/W4` | GCC, Clang, CI lint. Adding a value to a scoped enum breaks *every* switch over it, including ones the change never touched, so `rg` the enum name and add the case to each. `-Wswitch-enum` ignores `default:`, so a catch-all does not cover it — and a `break`-only case placed next to another `break`-only case then trips `bugprone-branch-clone`, so put it where its neighbour has a body |
| `-Woverloaded-virtual` where a JUCE base declares one name TWICE | silent — MSVC's C4264 is off at `/W4` | **GCC only** — `ComponentMovementWatcher` declares the pure virtual `componentMovedOrResized(bool, bool)` a subclass must implement AND its own public `ComponentListener` override `componentMovedOrResized(Component&, bool, bool)`; declaring only the former hides the latter. Clang carries `-Woverloaded-virtual` in the same flag set and still stays quiet, so macOS CI does not reproduce it either. Name the base's overloads back into scope with `using Base::method;` — that unhides them, where a suppression would leave them unreachable |
...
| `bugprone-unchecked-optional-access` | partial against the MSVC STL, which is worse than silent: a local run reports some sites and misses others in the same sweep (a large Catch2 suite reported none while CI found four), so a clean local run proves nothing | CI lint — and not always every platform's: a read inside a loop nested under its guard is lost to the loop, and only Linux (libstdc++) reported one that macOS passed. Hoist the read above the loop into a named `const`; the value is invariant across the scan anyway |
...
| A guard and its access reached through *separate* expressions | silent | CI lint — `bugprone-unchecked-optional-access` cannot tie `if (v[0].opt.has_value())` to `v[0].opt->x`, nor one call of an accessor to the next: they are distinct calls that it cannot prove yield the same object. Adding the `if` guard is not enough; bind the element **once** to a named reference and guard that name. Binding once is also not enough on its own: a `REQUIRE` on the bound name is still invisible, so the explicit `if` must follow it |
| `bugprone-optional-value-conversion` | not reported, and a per-file run against the MSVC STL stays silent on it too, so Windows lint passes while macOS and Linux fail | CI lint — `opt_a = *opt_b` inside an `if (opt_b.has_value())` unwraps and immediately re-wraps. Assign the optional itself (`opt_a = opt_b`), keeping the `if` where its absence would otherwise clear the target. The same holds for an argument: `f(*opt)` into a `std::optional` parameter is `f(opt)` |
| `bugprone-incorrect-roundings` | not reported: lint is on-demand | CI lint — `static_cast<int>(x + 0.5f)` is the hand-rolled round. Use `static_cast<int>(std::lround(x))`; for the non-negative quantities this codebase rounds, the value is unchanged |
| A clang-tidy check that fires only against the *MSVC* STL | **reported locally, absent from macOS/Linux CI** — the one class that runs the other way | Windows CI lint only — `cppcoreguidelines-pro-type-member-init` fires on a record whose implicit default constructor is non-trivial (a `std::span` member) while its scalar fields stay bare; libc++ and libstdc++ make that constructor trivial and say nothing. A local finding macOS CI does not repeat is therefore still real |
| A `NOLINTNEXTLINE` whose justification runs onto a second comment line | never reported: lint is on-demand | CI lint — the marker covers only the line *immediately* after itself, so a trailing explanation line absorbs it and the suppression silently does nothing. Put the prose first and the marker last |
...
| A pixel probe whose ink is a GLYPH | DirectWrite fills the stroke, so an exact-white test passes | macOS CI tests — CoreText leaves a digit of lane size a hair under full coverage, and FreeType (Linux) fills it like Windows, so the platform that disagrees is the one no local build and no per-file lint sweep reaches. Rasterized geometry (a `drawLine` stroke, a fill) is the software renderer's and matches everywhere; a glyph is not. Probe the mark, not the text over it — and when the text IS the subject, classify each painted pixel to the nearer of the two known colors instead of testing equality. A probe that silently drifts ONTO a glyph is the trap: a drawing rule that newly paints a head over the spot being probed leaves the assertion passing on two platforms for a reason it never meant |
| Any clang-tidy check on newly written code (`performance-enum-size`, `cppcoreguidelines-pro-bounds-constant-array-index`, `bugprone-branch-clone`, `misc-const-correctness`, `modernize-use-auto`, `modernize-use-std-numbers`, `readability-identifier-naming`, `bugprone-misplaced-widening-cast`, `bugprone-integer-division`, `modernize-use-designated-initializers`, `modernize-avoid-c-arrays`, `modernize-min-max-use-initializer-list`, `modernize-use-integer-sign-comparison`, `modernize-raw-string-literal`, `modernize-use-emplace`, `performance-no-automatic-move`, `performance-faster-string-find`, `performance-unnecessary-copy-initialization`, `performance-unnecessary-value-param`, `performance-inefficient-vector-operation`, `bugprone-exception-escape`, `cppcoreguidelines-prefer-member-initializer`, `cppcoreguidelines-missing-std-forward`) | never reported: lint is on-demand, so no build runs it | CI lint on all three platforms — for the flow-insensitive classes above, one file reproduces CI without the machine-saturating whole-project target: `clang-tidy -p build/release --extra-arg=-Wno-unknown-warning-option --extra-arg=-Wno-unused-command-line-argument <file>`. Clean per-file output is still not proof: the two dataflow checks above stay unreliable against the MSVC compile database |

The *compiler* half of this table is runnable too, not just a reading pass. Append to that per-file command
an EXPLICIT list of `clang-diagnostic-<warning>` checks (never the `clang-diagnostic-*` wildcard, which drags
in `-Wc++98-compat` noise), pass each warning itself as `--extra-arg=/clang:-W<warning>` — a named check only
filters what is shown and enables nothing — and pass `--extra-arg=/clang:-Wno-error`. That last one is not
optional: the compile database carries `/WX`, so the first warning in the TU (JUCE's headers always supply
one) becomes an error, and once an error exists clang skips its end-of-TU unused-declaration pass, silently
hiding every `-Wunused-function`. For a Catch2 test TU also pass
`--extra-arg=/DCATCH_INTERNAL_START_WARNINGS_SUPPRESSION=_Pragma("clang diagnostic push")` and the matching
`/DCATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION=_Pragma("clang diagnostic pop")`, which give Catch2 back the
push/pop it uses on macOS so its `-Wshadow` suppression stays inside its own macros. With all of these, the
whole Clang/`-Werror` flag set CI uses can be swept
locally — over every project TU when a struct gained a field or an enum an enumerator, since those break
files the change never touched. It does NOT cover the **GCC-only** rows above — `-Wrange-loop-construct` and
compound-assignment `-Wsign-conversion` stayed silent under Clang and still stopped the Linux build — nor
`-Wfloat-equal` against an exactly representable literal (`x == 0.5`), which Clang deliberately skips and
GCC still reports.
