...
| `bugprone-optional-value-conversion` | not reported: lint is on-demand, and the construct compiles everywhere | CI lint — `opt_a = *opt_b` inside an `if (opt_b.has_value())` unwraps and immediately re-wraps. Assign the optional itself (`opt_a = opt_b`), keeping the `if` where its absence would otherwise clear the target |
