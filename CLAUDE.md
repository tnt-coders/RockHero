...
The *compiler* half of this table is runnable too, not just a reading pass. Appending an EXPLICIT list of
`clang-diagnostic-<warning>` checks to that per-file command (never the `clang-diagnostic-*` wildcard, which
drags in `-Wc++98-compat` noise) makes clang-tidy enable those warnings against the MSVC compile database, so
the whole Clang/`-Werror` flag set CI uses can be swept locally over just the changed TUs. It does NOT cover
the **GCC-only** rows above — `-Wrange-loop-construct` and compound-assignment `-Wsign-conversion` stayed
silent under Clang and still stopped the Linux build.
