...
- Enforce FLAC on the package **write** path too: the reader rejects non-`.flac` audio
  (`rock_song_package_read.cpp`, `hasFlacExtension`) but the writer copies audio with its source
  extension, so it can produce a package the reader refuses. Verified 2026-07-15.
