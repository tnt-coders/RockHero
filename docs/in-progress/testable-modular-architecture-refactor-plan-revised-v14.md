# Testable Modular Architecture Refactor Plan (Revised v14)

## Purpose of This Revision

Supersedes `testable-modular-architecture-refactor-plan-revised-v13.md`. v14 fixes three
remaining implementation-contract details: load errors have an explicit lifecycle, the
view presents load errors on an edge instead of repeatedly on every state push, and the
thumbnail-empty case is removed from this plan because no unload flow is in scope. It
also adds an explicit goal-alignment checkpoint against the original architecture
objectives. No architectural moves change.

### Fixes Introduced in v14

1. **Load errors persist until success or explicit dismissal.** v13's derive path could
   accidentally clear `last_load_error` on an unrelated transport event because
   `TransportState` and `currentAsset()` do not carry error state. v14 pins the rule:
   non-load transport events preserve the cached `last_load_error`; successful loads
   clear it; a future explicit error-dismiss intent may clear it, but this plan does
   not add that intent.
2. **Error rendering is edge-triggered in the view.** If the view renders errors with
   `juce::NativeMessageBox`, it must not reopen the same modal on every cursor update.
   `EditorView::setState` presents an error only when `last_load_error` changes from
   empty to present, or when the present error string changes.
3. **No thumbnail clear path in this refactor.** v13 mentioned a clear-thumbnail
   method even though this plan does not add unload. v14 removes that method and the
   empty-asset branch. The loaded asset is absent only in the initial no-file state,
   before any thumbnail source has been assigned.
4. **Goal alignment is explicit.** v14 adds a short checkpoint showing that the plan
   still targets the original reasons for the refactor: testable editor workflow,
   framework isolation, thin adapters, app-level composition, and incremental delivery.

### Fixes Preserved From v13

1. **Failed replacement loads preserve the prior asset.** v12 said `Engine` should
   update `currentAsset()` at the start of `loadAudioAsset(...)` and clear it on
   failure. That contradicted the preserved-prior-load UX: a failed replacement load
   must leave the existing audio and waveform visible. v13 pins the rule: validate and
   load into a candidate first, then replace `currentAsset()` only after success. On
   failure, leave `currentAsset()` and the existing transport state untouched unless
   the failure belongs to an explicit unload operation, which this plan does not add.
2. **Presenter derivation avoids tick-rate `AudioAsset` copies.** v12 kept asset
   identity out of `TransportState` to avoid allocating on every position tick, but a
   naive derive step would still copy `std::filesystem::path` into a fresh
   `EditorViewState` every event. v13 requires the presenter to reuse the cached
   `loaded_asset` when `currentAsset()` is unchanged and only copy the asset when its
   value changes.
3. **Compared value types define equality.** v12 relied on comparing
   `EditorViewState`, `AudioAsset`, and optional assets without specifying equality.
   v13 adds defaulted `operator==` declarations to the value-type sketches whose values
   are compared.
4. **Step 5 includes its transitional app rewire.** v12 claimed every step is
   independently compilable while changing `WaveformDisplay` before `EditorView`
   exists. v13 makes step 5 update the existing `MainWindow::ContentComponent`
   temporarily: it creates and attaches the thumbnail, converts normalized seek to
   seconds, and drives the waveform cursor from the existing engine listener until
   step 7 retires `ContentComponent`.

### Fixes Preserved From v12
...
1. **Asset identity is a separate query on `ITransport`, not a field of `TransportState`.**
   ...
   to `ITransport`. The presenter checks it during every derive step, but per v13 only
   copies the value when the asset changes. Returning by const reference keeps the
   query itself tick-free regardless of call rate; `AudioAsset` can grow without
   affecting tick cost when the current asset is unchanged.

2. **`EditorViewState` carries `std::optional<core::AudioAsset> loaded_asset`.** The
   presenter derives it from `currentAsset()`, reusing the cached optional when the
   asset is unchanged and copying only on asset changes. `EditorView::setState`
   compares it against the last state it rendered and, on change, forwards the asset to
   `WaveformDisplay::setThumbnailSource(const core::AudioAsset&)`, which in turn calls
   `audio::Thumbnail::setSource(asset)`. This replaces today's path where
...
4. **`Engine::loadAudioAsset(...)` must publish a fresh `TransportState` on success.**
   ...
   new asset after validation succeeds; the snapshot must not fire until that ordering
   is in place (new `currentAsset()` visible before listeners observe the new
   `TransportState`). Failed replacement loads leave the prior `currentAsset()`
   untouched.
...
1. **Presenter cache seeding is explicit.** `EditorPresenter` seeds its cached
   `EditorViewState` from `transport.state()` and `transport.currentAsset()` in the
   constructor; listener callbacks update the cache; `attachSink` flushes the cache.
...
listeners re-deriving in response see the new asset. On failure, implementations must
not clear or replace `currentAsset()` for a replacement load; the previously loaded
asset remains authoritative until a later successful load.
...
- Tick-rate asset rule: the derive step must not copy `core::AudioAsset` on every
  position event. It compares `transport.currentAsset()` to the cached
  `EditorViewState.loaded_asset`; if equal, the cached optional is reused. Because
  this plan has no unload flow, the asset value is copied only when the optional
  changes from empty to present or one present asset value to another.
- Load-error lifecycle rule: ordinary transport events preserve the cached
  `last_load_error`. Only a successful load clears it. A future explicit dismiss
  intent may also clear it, but this refactor does not add that intent.
...
3. If `state.loaded_asset != m_last_rendered_state.loaded_asset`, calls
   `m_waveform_display.setThumbnailSource(*state.loaded_asset)`. The optional is absent
   only in the initial no-file state, before any thumbnail source has been assigned, so
   no clear-thumbnail operation is part of this refactor.
4. Renders `state.loaded_asset_label` and `state.last_load_error` (today's
   ...
   another). Error presentation is edge-triggered: the view presents an error only when
   `last_load_error` changes from empty to present, or when the present string changes,
   compared with `m_last_rendered_state.last_load_error`.
...
- **On failure:** the presenter synthesizes an `EditorViewState` whose
  ...
  Later non-load transport events preserve that error. The view presents the error only
  on the edge where the optional becomes present or the message string changes.
...
2. **Make `audio::Engine` implement `audio::ITransport`.** Keep Tracktion wiring
   ...
   from `currentAsset()`. `loadAudioAsset(...)` validates and inserts the candidate
   asset first; only after success does it replace the stored current asset. On failure,
   leave the stored current asset and prior transport state intact. On a successful
   `loadAudioAsset(...)`, publish a fresh `TransportState` snapshot to listeners even
   when `playing` and `position` are unchanged, and do so *after* the asset member is
   updated so listeners observe the new `currentAsset()` on their first re-derive.
   During transition the engine dispatches to both `Engine::Listener` (legacy, pointer
   registration) and `ITransport::Listener` (new, reference registration).
   Dual-dispatch ends in **step 7**, when `Engine::Listener` is deleted.

3. **Rename `audio::Thumbnail::setFile(const juce::File&)` to
   `audio::Thumbnail::setSource(const core::AudioAsset& asset)`**. Update
   `TracktionThumbnail` and all call sites. Until step 5 replaces
   `WaveformDisplay::setAudioFile(...)`, that method remains as a temporary UI-facing
   adapter and converts its `juce::File` argument to `core::AudioAsset` before calling
   `Thumbnail::setSource(...)`. Remove the now-unused `juce::File` forward declaration
   from `thumbnail.h`. This step adds a second reason `rock-hero-audio` depends on
   `rock-hero-core`; the dependency-line correction in `architecture.md` already covers
   it.
...
5. **Refactor `WaveformDisplay`** to receive its `audio::Thumbnail` via
   ...
   from an `audio::Engine&`. Add `setCursorProportion(double)` and
   `setThumbnailSource(const core::AudioAsset&)`.
   ...
   widget. To keep this step independently compilable, temporarily update the existing
   `MainWindow::ContentComponent` while it still exists:
   - construct `WaveformDisplay` without an engine,
   - call `m_waveform_display.attachThumbnail(m_audio_engine.createThumbnail(
     m_waveform_display))` during component construction,
   - keep `ContentComponent`'s `Engine::Listener` registration until step 7 and have
     `engineTransportPositionChanged(...)` call
     `m_waveform_display.setCursorProportion(seconds / loaded_length)` using a cached
     loaded length read from `m_audio_engine.state().length.seconds` after successful
     loads, with a zero-length guard that sends cursor proportion 0.0,
   - update the waveform seek callback to receive normalized x and call
     `m_audio_engine.seek(normalized_x * loaded_length)` only when the cached loaded
     length is greater than zero, and
   - update the successful-load path to build one `core::AudioAsset`, call
     `m_audio_engine.loadAudioAsset(asset)`, then call
     `m_waveform_display.setThumbnailSource(asset)` and cache the accepted length used
     by the transitional seek/cursor math.
   The second `Engine::Listener` implementer, `ContentComponent`, is retired in step 7.
...
7. **Extract `ui::EditorView` from `MainWindow::ContentComponent` and rewire
   ...
   - Move `ContentComponent`'s responsibilities into
     ...
     `WaveformDisplay::setThumbnailSource(...)`.
...
TEST_CASE("EditorPresenter: failed replacement load surfaces error and preserves prior flags")
{
    ...
    // A later transport event must still preserve the prior asset; failed replacement
    // loads do not clear currentAsset().
    transport.simulateState({.file_loaded = true,
                             .playing = false,
                             .position = {0.0},
                             .length = {10.0}});
...
When `EditorView` lands in step 7, add one focused UI wiring test or equivalent
component-level check for the edge-triggered load-error rule: applying the same
`last_load_error` twice must not present the error twice, while changing the error
string may present again. This is deliberately a narrow view-sink test; the primary
workflow tests remain the headless presenter tests above.
...
The plan remains aligned with the project architecture goals:

- **Automated-testable workflow:** load, play/pause, stop, seek, failed-load handling,
  cursor derivation, and loaded-asset state are handled by `EditorPresenter` and
  tested with `FakeTransport` and `FakeEditorViewSink`. These tests do not require
  JUCE initialization, Tracktion runtime state, an audio device, or a message loop.
- **Framework isolation:** `rock-hero-ui` consumes `audio::ITransport` and
  `audio::Thumbnail` headers, not `audio::Engine` or Tracktion types. `juce::File`
  conversion is isolated to the view's native-dialog callback.
- **Thin audio adapter:** `rock-hero-audio` owns Tracktion-backed transport,
  thumbnail/proxy generation, and framework translation. It does not take on editor
  workflow, view-state policy, or UI presentation rules.
- **Presentation-focused UI:** `EditorView` renders `EditorViewState`, owns JUCE
  widgets, emits user intents, and performs edge-triggered presentation effects such
  as load-error dialogs. It does not decide transport policy or seek math.
- **Small app composition root:** `MainWindow` creates the concrete engine, presenter,
  view, and thumbnail, wires them in lifetime-safe order, and keeps business behavior
  out of the app target.
- **Incremental delivery:** the seven-step order keeps each step buildable, including
  the temporary `ContentComponent` rewire while `WaveformDisplay` changes before
  `EditorView` exists.
- **Deferred complexity stays deferred:** pitch detection, scoring, plugin-host,
  repository, waveform-data-source, cross-thread transport, and simulation ports remain
  out of scope until their first real consumers exist.

The plan is intentionally narrow: it establishes the editor transport seam and
presenter pattern now, while preserving the larger architecture direction toward pure
core/gameplay logic, thin framework adapters, explicit time/state, and headless tests.
...
- Tick-rate allocation via asset identity baked into `TransportState`. *Addressed:
  ...
  reference; the presenter also reuses its cached `loaded_asset` when the current
  asset is unchanged, so `AudioAsset` can grow without affecting the tick-rate path.*
...
- `testable-modular-architecture-refactor-plan-revised-v12.md` — v12, superseded by
  this v14.
- `testable-modular-architecture-refactor-plan-revised-v13.md` — v13, superseded by
  this v14.
