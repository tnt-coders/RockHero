...
[Charter](https://github.com/Lordszynencja/Charter) is an MIT-licensed source-game-style chart
...
- **One true tab per arrangement — no difficulty levels.** Decision 2026-07-05: Charter carries
  source-game-style per-difficulty level lists (pared-down variants of each arrangement); RockHero
...
- **One physical onset = one note event; no link-next flag.** Decision 2026-07-05.
  Charter/the source game model tied notes as separate events joined by `linkNext`; 26 Charter files
...
4. **Authoring comes later.** Note editing and import (Guitar Pro / the source game XML) are separate
...
- **Notes say what sounds; shapes say what the hand holds; templates are reusable postures.**
  ...
  and arpeggios with a single mechanism (the source game data confirmed chords and handshapes are ~1:1
...
- **Sections.** Navigation/practice markers; the type vocabulary starts small and grows as
  needed (no commitment to Charter's 31 kinds; the source game's name+number pairs collapse to type,
...
- **The format is string-count-generic; the target is at least 10 strings.** `tuning.strings`
  ...
  String colors extrapolate the source game's own system rather than adopting Charter's: the source game's
...
- **Quarter bends (and finer) are already representable.** Bend pair values are plain numbers in
  ...
  granularity needs no format change. the source game's coarseness is an importer limitation, not a
  format one.
...
The format was pressure-tested against a 39-song the source game corpus (BTBAM epics, Opeth, Yes,
...
audio, tone regions generated from the source game tone changes with empty tone documents ready for
...
- **The tempo map compresses hostile grids.** Hand-warped charts (Marigold: 943 individually
  timed beat markers, meters alternating 7/4 and 4/4 per measure) reduce to ~135 anchors + ~29
  signature entries.
...
Importer findings recorded for the future import plan: the source game `bendValue.step` is already in
