...
- **One `sounds` stream per difficulty level.** A sound is a note or a chord instance (sum type in
  ...
  the format shape must allow multiple levels because source-game-style dynamic difficulty and the
...
4. **Authoring comes later.** Note editing, import (Guitar Pro / the source game XML), and difficulty
