...
For imported source-package arrangements, seed `Arrangement::id` from the manifest `PersistentID` when it is
...
For authored Rock Hero content that does not come from source-package, generate the same kind of stable
...
7. Add focused tests.
   - source-package importer preserves `PersistentID` as arrangement ID.
...
- Whether source metadata such as source-package `PersistentID`, `MasterID`, `ManifestUrn`, or `DLCKey`
