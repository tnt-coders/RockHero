...
the source game source-package packages provide useful precedent: the extracted sample package uses readable
...
source-package IDs into the Rock Hero package format.
...
Generate a fresh Rock Hero ID whenever an object is created, including during source-package import. Use a
...
The Rock Hero package format is its own format. source-package-specific identifiers
...
(for example, to disambiguate arrangements within a single source-package) but do not survive into the
saved package. This keeps the format self-contained and free of coupling to any external tooling or
identity scheme.

Note on source arrangement chart payloads: Rock Hero currently embeds source-package's source arrangement chart format
...
- This is closer to the source-package resource-name style.
...
2. Update source-package import identity.
...
7. Add focused tests.
   - source-package importer generates a fresh Rock Hero ID per arrangement and writes no source-package
...
- Whether to embed a format/version marker in `song.json` (e.g. `"arrangementFormat":
  "source-package-xml-v1"`) ahead of a future Rock Hero-native arrangement format. Not needed until a
