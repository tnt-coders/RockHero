...
- `rock-hero-editor/core/include/rock_hero/editor/core/source-package_song_importer.h`
...
- `audio_device_status_text.h`: this is display-projection policy. Consider keeping only durable
  ...
  `src/` helper.
- `rock_song_importer.h` and `source-package_song_importer.h`: these are concrete default importers. The
...
4. If they are editor-core defaults, move `RockSongImporter` and `source-packageSongImporter` private.
