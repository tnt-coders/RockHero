# Testable Modular Architecture Refactor Plan (Revised v8)

## Purpose of This Revision

Supersedes `testable-modular-architecture-refactor-plan-revised-v7.md`. v8 closes a
final round of inconsistencies found on re-read of v7. None of the architectural moves
change; the revisions are internal consistency fixes.

### Fixes Introduced in v8
...
- Legacy `Engine` public methods redundant with `ITransport` are removed in step 7.
...
6. `EditorView::setPresenter(EditorPresenter& presenter)` stores `&presenter`; the
   ...
   (engine → view → presenter) satisfies this: presenter is destroyed before view.
...
7. **Extract `ui::EditorView` from `MainWindow::ContentComponent` and rewire
   ...
   - Rewire `MainWindow` to compose `Engine` → `ITransport&` → `EditorPresenter` →
     ...
     puts the engine before the view. Preserve
...
- Raw-pointer lifetime coupling across ownership boundaries. *Addressed: thumbnail
  ...
  view bounded by `MainWindow` member order.*
...
- `testable-modular-architecture-refactor-plan-revised-v7.md` — v7, superseded by
  this v8.
