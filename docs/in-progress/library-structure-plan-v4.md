# Library Structure Plan (v4)

Status: proposed. Supersedes `library-structure-plan.md`,
`library-structure-revised-plan.md`, and `library-structure-plan-v3.md`.
...
That split was useful early because it separated domain, audio, and UI concerns. It now hides a
second axis that matters just as much: product scope.
...
Some code in the current shared-looking libraries is editor-only today. Future game-specific code
also needs a clear home that is not accidentally shared. The risk is not that product-specific
code exists. The risk is that folders, targets, namespaces, and includes do not communicate
ownership, so editor code can drift into common modules and common modules can become broad link
blobs.

There is also a newer editor app-shell library under `apps/rock-hero-editor/`. It currently keeps
`MainWindow`, `EditorSettings`, and settings tests outside the executable target, which is good
for testability, but the library still lives under `apps/`. That makes `apps/` more than a thin
process-startup area and creates a target-name collision with the proposed editor umbrella.

The structure needs to answer three questions for every file:

1. Which product scope owns this code? Shared, editor-only, or game-only?
2. Which technical layer does it belong to? Core, audio, or UI?
3. Is it a headless/testable layer or a framework-bound adapter/presentation layer?

## Goals

- Keep the `core`, `audio`, and `ui` mental model.
- Make shared, editor-only, and game-only ownership visible in the folder path.
- Keep app targets as thin composition roots.
...
- Provide strong placement guidance for future code and AI-assisted edits.
- Preserve the user-facing game executable name `rock-hero`.

## Proposed Structure

Use three product-scope parent folders under `libs/`. Each parent owns the same three layer cells:
`core`, `audio`, and `ui`.
...
All nine submodules are created up front as real targets. Empty submodules are dependency-free
stubs with `README.md` guardrails that describe what belongs there, what must not go there, and
what kind of first real code would justify filling the cell.
...
The current editor app-shell library under `apps/rock-hero-editor/` is initially moved here. That
includes `main_window.*`, `editor_settings.*`, and the settings tests. If `EditorSettings` is later
rewritten to be framework-free or becomes part of broader editor workflow policy, it can move to
`editor/core` in a separate focused change.
...
Each top-level submodule has a target, namespace, and include path that mirrors its product scope
and layer.
...
editor -> common only
game   -> common only
apps   -> common + their matching product scope
```
...
- The app target coordinates process-level event-loop and focus concerns across GUI systems.

## Stub Conventions

All nine top-level submodules are created up front. Empty submodules follow these rules:
...
- product-scope parent folders
- nine `core` / `audio` / `ui` cells
...
### Phase 2: Create Parent Layout, Umbrellas, And Stubs

Create:

```text
libs/rock-hero-common/
libs/rock-hero-editor/
libs/rock-hero-game/
```

Create all nine submodule folders and targets:
...
Create README guardrails in every submodule. Existing code can remain in old locations during this
phase if needed.

### Phase 3: Move `rock-hero-core` Into `common/core`

Move the current core library into `libs/rock-hero-common/core/`.
...
Move current audio code into `libs/rock-hero-common/audio/`.
...
Move concrete editor JUCE presentation code into `libs/rock-hero-editor/ui/`.
...
Move headless editor workflow into `libs/rock-hero-editor/core/`.
...
### Phase 7: Move The Editor App-Shell Library Out Of `apps`

Move the current static library code from `apps/rock-hero-editor/` into
...
After this phase, `apps/rock-hero-editor/` should contain only the executable entry point and
process-startup CMake wiring. The existing `rock_hero_editor` app-shell library target is removed
or replaced by the `rock_hero::editor` product umbrella, so there is only one meaning for the
editor umbrella target.
...
### Phase 8: Audit Project And Package Persistence
...
### Phase 9: Fill Remaining Stubs As Code Arrives
...
### Phase 10: Post-Migration API/Impl Split Review
...
### Phase 11: Rename Game App Folder And Internal Targets

For long-term symmetry, rename:

```text
apps/rock-hero/ -> apps/rock-hero-game/
```

Rename internal CMake targets to the `rock_hero_game_*` shape.

Preserve the shipped executable output name:

```cmake
set_target_properties(rock_hero_game_exe PROPERTIES OUTPUT_NAME "rock-hero")
```

This phase is part of the plan, but it should be kept separate from the core library moves to keep
...
- all nine scope/layer submodules exist under `libs/`
- every submodule has a README guardrail
- `common/audio` has `api`, `impl`, and umbrella targets
- parent umbrellas aggregate only their own scope
- apps link `common + editor` or `common + game`
...
- editor app-shell library code and tests no longer live under `apps/rock-hero-editor/`
- `apps/rock-hero-editor/` contains only executable startup and process composition wiring
- permanent compatibility aliases and forwarding headers are removed
- `apps/rock-hero-game/` exists while the executable output remains `rock-hero`
- durable design docs describe the implemented structure

## Open Questions

- Exact README template for each submodule.
- Exact placement of individual `.rhp` and `.rock` persistence files after the Phase 8 audit.
