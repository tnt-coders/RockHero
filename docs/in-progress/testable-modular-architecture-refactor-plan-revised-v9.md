# Testable Modular Architecture Refactor Plan (Revised v9)

## Purpose of This Revision

Supersedes `testable-modular-architecture-refactor-plan-revised-v8.md`. v9 fixes two
remaining implementation-contract inconsistencies: `Engine` keeps its public
`ITransport` overrides, and `MainWindow` member order now makes the view die before the
presenter whose pointer it stores.

### Fixes Introduced in v9

1. **Legacy `Engine` method deletion clarified.** v8 said to delete `Engine` public
   methods including `play`, `pause`, and `stop`, but those names still exist as
   public `ITransport` overrides. v9 clarifies that only legacy non-interface back
   doors are removed; `Engine` retains the public `ITransport` override surface and
   concrete factory methods such as `createThumbnail(...)`.
2. **Presenter/view lifetime corrected.** v8 said `EditorView` stores an
   `EditorPresenter*` and that the presenter must outlive the view, but the proposed
   member order destroyed the presenter first. v9 changes the required `MainWindow`
   member order to `engine`, `presenter`, `view`, so destruction runs
   `view -> presenter -> engine`.

### Fixes Preserved From v8

1. **`EditorView` presenter-binding consistency.** v7's sketch had
   `setPresenter(EditorPresenter&)` but also `m_presenter->onPlayPausePressed()` in the
   key-listener, mixing reference and pointer. v8 stores the presenter as a pointer
   (`EditorPresenter* m_presenter{nullptr}`), binds it by taking the setter's argument's
   address, and the key-listener uses `m_presenter->...` consistently.
2. **`EditorView` inheritance is stated.** `EditorView` inherits `juce::Component`,
   `juce::KeyListener`, and `IEditorViewSink`. This mirrors today's `ContentComponent`
   (which inherits `juce::Component`, `juce::KeyListener`, and `audio::Engine::Listener`)
   minus the retired engine-listener role, plus the sink role that replaces it.
3. **`TransportControls` has two buttons, not three.** v7's step 6 said "three button
   callbacks." The widget has a play/pause toggle button and a stop button —
   two intents: `onPlayPausePressed()` and `onStopPressed()`. v8 corrects the step.
4. **Presenter pushes initial state on `attachSink`.** Without this, the view renders
   default-constructed `EditorViewState` until the first transport event fires, which
   looks like a load-button-only UI even when a file is already loaded. v8 adds the
   rule and the corresponding test.
5. **Concurrency contract phrasing.** v7 said `ITransport::state()` and its listener
   callback are "message-thread only," which reads oddly in a single-threaded unit test.
   v8 states the real constraint: no concurrent access from multiple threads, and in
   the production app both are invoked on the JUCE message thread. Single-threaded
   tests trivially satisfy the contract.
6. **`MainWindow::~MainWindow` still needs `clearContentComponent()`.** JUCE's
   `DocumentWindow` holds a non-owning pointer to whatever was passed to
   `setContentNonOwned`. The composition root uses a `std::unique_ptr<EditorView>`;
   the destructor must null out JUCE's pointer before the unique_ptr destroys the view.
7. **Legacy dual-dispatch has an endpoint.** Step 2 runs the engine's legacy
   `Engine::Listener` alongside the new `ITransport::Listener`. Step 7 is the endpoint:
   `Engine::Listener` is deleted there, which ends the dual-dispatch.
...
- View sink: `attachSink(IEditorViewSink&)`. On first attach (and on every attach),
  the presenter immediately pushes a derived `EditorViewState` built from the current
  transport state. This keeps the view correct when composition order attaches the
...
4. **Add `ui::EditorPresenter`, `ui::EditorViewState`, `ui::IEditorViewSink` with
   ...
   The presenter registers/unregisters as a transport listener in its ctor/dtor, and
   `attachSink` immediately pushes an initial `EditorViewState` derived from the
   current transport state. This lands the testing story before any JUCE widget
   changes.
...
6. **Refactor `TransportControls`** to render from `EditorViewState`. The widget has
   two buttons (play/pause toggle + stop) and exposes two callbacks —
   `on_play_pause_pressed` and `on_stop_pressed` — which `EditorView` forwards to
   `presenter.onPlayPausePressed()` and `presenter.onStopPressed()`. Remove the
   internal state machine (`updateButtonStates`, `m_is_playing`, `m_file_loaded`,
   `m_transport_position`, `isFileLoaded()`, `setFileLoaded()`, `setPlaying()`,
   `setTransportPosition()`, and the internally-dispatching `onPlayPauseClicked()`).
   Button enabledness and the play/pause icon are driven by the incoming
   `EditorViewState`.
...
7. **Extract `ui::EditorView` from `MainWindow::ContentComponent` and rewire
   ...
   - Move `ContentComponent`'s responsibilities into
     ...
     to the presenter via a stored `EditorPresenter*`.
   - Rewire `MainWindow` to compose `Engine` → `ITransport&` → `EditorPresenter` →
     ...
     puts the engine before both presenter and view, and puts the presenter before the
     view. Preserve
...
- Raw-pointer lifetime coupling across ownership boundaries. *Addressed: thumbnail
  ...
  view bounded by corrected `MainWindow` member order.*
...
- `testable-modular-architecture-refactor-plan-revised-v8.md` — v8, superseded by
  this v9.
