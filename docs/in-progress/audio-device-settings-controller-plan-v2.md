# Audio Device Settings Controller Plan (v2)
...
- `rock-hero-common/audio`: shared audio-device settings state, port, and backing implementation.
...
- The typed-error convention is **not** new — it is already documented in
  `docs/design/coding-conventions.md` under "Recoverable Error Returns" and reinforced by
  `docs/design/architectural-principles.md` under "Typed Boundary Errors". The plan now cites the
  existing rule rather than proposing a future migration.
- `StereoOutput` renamed to `StereoOutputPair`. The type carries two channel indices plus display
  text, so the pair meaning belongs in the name. `StereoOutput` alone read as a generic
  audio-routing concept.
- External backend refresh lifecycle is now explicit. `IAudioDeviceSettings` exposes a `Listener`
  surface following the existing `IAudioDeviceConfiguration::Listener` pattern; the controller
  subscribes and re-derives view state on notification.
- Test-plan typo (`I- AudioDeviceSettings ...`) corrected.
...
- Stable enum for branchable settings failures.
- Follows the project's existing typed-error convention documented in
  `docs/design/coding-conventions.md` under "Recoverable Error Returns".
...
- Typed error value with `AudioDeviceSettingsErrorCode code` and `std::string message`.
- Mirrors the shape used by `PluginHostError`, `LiveRigError`, `ProjectError`, `ArchiveError`,
  `SongImportError`, and `SongPackageError`. Marked `[[nodiscard]]` and uses constructors for
  default and contextual messages per the existing convention.

`StereoOutputPair`

- Project-owned value with left channel index, right channel index, and display text.
...
- Lives in common/audio without a settings-specific qualifier because the concept is a general
  stereo-routing value, not a settings-only one. The `Pair` suffix is kept so the type name
  conveys the channel-pair meaning explicitly.

`AudioDeviceSettingsState`

- Shared snapshot returned by the settings port.
- Contains available audio systems, device names, channel choices, stereo output pairs,
  sample rates, buffer sizes, row-shape flags such as separate input/output devices, capability
  flags for test output and control panel, the current staged selection fields directly (audio
  system name, input/output device names, input channel index, stereo output pair, sample rate,
  buffer size), and the last error when an operation failed.
...
- Fallible operations return `std::expected<T, AudioDeviceSettingsError>` per the existing
  typed-error convention.
- Uses only project-owned settings values in its public contract.
- Defines a nested `Listener` interface following the existing `IAudioDeviceConfiguration::Listener`
  pattern: one notification method `onAudioDeviceSettingsChanged()` invoked on the message thread
  after the underlying backend state changes. `addListener` and `removeListener` mirror the
  existing port. This keeps settings-refresh behavior inside the common/audio boundary and lets the
  editor controller subscribe directly rather than threading refresh intents through composition
  glue.

`AudioDeviceSettings`

- JUCE-backed common/audio implementation of `IAudioDeviceSettings`.
- Wraps the existing `IAudioDeviceConfiguration` / `juce::AudioDeviceManager` behavior, subscribes
  to the underlying configuration listener, and forwards notifications to its own listeners.
...
- Receives editor settings intents such as selected audio system ID, selected device ID, selected
  input channel ID, selected stereo output pair ID, selected sample-rate ID, selected buffer-size
  ID, OK, Cancel, Test Output, and Control Panel.
- External backend refresh is not an explicit intent on this contract; the controller listens to
  `IAudioDeviceSettings` directly and re-derives view state when notified.

`AudioDeviceSettingsController`

- Editor-core workflow controller for one audio-device settings window.
- Subscribes to `IAudioDeviceSettings::Listener` for the lifetime of the window and re-derives
  `AudioDeviceSettingsViewState` whenever the backend reports a change. Unsubscribes on destruction.
...
## External Backend Refresh

While the settings window is open, the underlying audio backend can change for reasons outside the
window (a USB device unplug, sample-rate change from another app on a shared backend, plugin scan
toggling driver availability). The new shape handles this through the port's own listener:

1. `IAudioDeviceSettings` exposes a `Listener` and broadcasts `onAudioDeviceSettingsChanged()`
   after the backing state changes.
2. `AudioDeviceSettings` (the concrete) subscribes to the existing
   `IAudioDeviceConfiguration::Listener` and forwards its notifications to its own listeners after
   resnapshotting state. Staged selections survive the resnapshot when they remain valid; invalid
   selections fall back to the defaulting policy.
3. `AudioDeviceSettingsController` subscribes to the port during construction and unsubscribes
   during destruction. Its listener callback pulls a fresh `AudioDeviceSettingsState` and rederives
   `AudioDeviceSettingsViewState`, which it pushes to `IAudioDeviceSettingsView::setState`.

This option was chosen over forwarding refresh intents through the window/composition layer
because it keeps the refresh semantics next to the rest of the settings boundary, mirrors the
existing `IAudioDeviceConfiguration::Listener` pattern that the project already uses for the same
backend, and avoids spreading the refresh contract across editor composition glue that the game
would need to reinvent.

## Implementation Plan

1. Add common/audio settings values (`StereoOutputPair`, `AudioDeviceSettingsState`,
   `AudioDeviceSettingsErrorCode`, `AudioDeviceSettingsError`), `IAudioDeviceSettings` with its
   nested `Listener`, and the `AudioDeviceSettings` implementation. Fold the existing free-function
...
2. Move the current staged-device logic out of `AudioDeviceSettingsView` into
   `AudioDeviceSettings` without changing behavior. `AudioDeviceSettings` subscribes to
   `IAudioDeviceConfiguration::Listener` and forwards notifications to its own listeners after
   resnapshotting its state.
3. Add editor-core settings controller/view contracts and `AudioDeviceSettingsViewState`.
   `AudioDeviceSettingsController` subscribes to `IAudioDeviceSettings::Listener` during
   construction.
...
5. Refactor `AudioDeviceSettingsWindow` and `EditorView` composition so `EditorView` no longer
   receives or stores a raw `juce::AudioDeviceManager*`. The window now takes an
   `IAudioDeviceSettings&` instead.
...
- `AudioDeviceSettings` initializes from an active backend route and derives available settings
  choices.
- Audio system, device name, channel, stereo output pair, sample-rate, and buffer-size selections
...
- Listener notifications fire after `IAudioDeviceConfiguration` broadcasts a change, after the
  staging transaction resnapshots state. Staged selections survive when still valid and fall back
  when invalid.
...
- Listener-driven refresh: when the port notifies, the controller pushes a fresh view state to
  `IAudioDeviceSettingsView` without requiring a user intent.
...
The [Editor Structure Assessment](#editor-structure-assessment) section above describes
project-wide controller and view-state guidance, not extraction-specific detail. After the
extraction proves the pattern, that guidance should migrate to
`docs/design/architectural-principles.md` so future contributors find the rules in the design docs
rather than buried in an in-progress plan. The structural principles in that file already cover
ports, adapters, view/state separation, and "humble object" extractions at a high level; the
addition would be the concrete decision rules for when a UI surface earns a controller or a
view-state type.