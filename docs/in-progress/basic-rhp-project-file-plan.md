...
- one Lead source arrangement chart file
- enough metadata to display and reload the project
- no the source game import, WEM conversion, plugin preset loading, or note authoring yet
...
metadata, asset lists, and arrangement references. Use XML for arrangement files because the source game
arrangements are XML and Rock Hero will eventually need XML arrangement parsing for import work.

Do not name the root file `manifest.json`. Reserve "manifest" for the source game import data and any
...
audio and later arrangement-specific overrides without duplicating paths in every source arrangement chart.
...
The first source arrangement chart should be a minimal placeholder with no editor-unused fields:
...
Audio assignment belongs in `song.json`, not in the source arrangement chart, because backing audio is
...
- Rock Hero `song.json`
- future the source game `.hsan`
- future the source game manifest `.json`

Add `pugixml` for:

- Rock Hero `arrangements/*.xml`
- future the source game `songs/arr/*.xml`
- future the source game `.xblock`
...
4. Parse source arrangement chart data needed by the current format.
...
- Add source arrangement chart parsing/writing for the minimal placeholder shell.
...
- Core tests for minimal placeholder source arrangement chart read/write.
...
- the source game source-package import.
- the source game ` audio-container` or ` audio-container` parsing.
- the source game `.hsan`, manifest JSON, `.xblock`, or aggregate graph parsing.
