# Testable Modular Architecture Refactor Plan (Revised v13)

## Purpose of This Revision

Supersedes `testable-modular-architecture-refactor-plan-revised-v12.md`. v13 fixes four
implementation-contract issues in v12: failed replacement loads must preserve the
previous `currentAsset()`, presenter derivation must not copy `AudioAsset` on every
position tick, value types that are compared must define equality, and step 5 must
include a transitional `ContentComponent` rewire so each refactor step remains
independently compilable. No architectural moves change.

### Fixes Introduced in v13
...
- Tick-rate asset rule: the derive step must not copy `core::AudioAsset` on every
  ...
  `EditorViewState.loaded_asset`; if equal, the cached optional is reused. The asset
  value is copied only when the optional changes from empty to present, present to
  empty, or one asset value to another.
