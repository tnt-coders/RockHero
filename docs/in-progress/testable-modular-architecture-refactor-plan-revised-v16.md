# Testable Modular Architecture Refactor Plan (Revised v16)

## Purpose of This Revision

Supersedes `testable-modular-architecture-refactor-plan-revised-v15.md`. v16 closes
contract gaps that v15 left implicit once the editor went multi-track-shaped, and
tightens several smaller points. The shape from v15 stands; v16 nails down behavior
the v15 prose either left to the reader or did not name at all.

### Fixes Introduced in v16
...
1. **`TransportState.length` scope is committed to single-track-applied semantics.**
   ...
   audio. v16 commits: in this revision, the editor applies audio to exactly one
...
2. **Engine must not invoke transport listeners during `applyTrackAudio(...)`.**
   ...
   would re-derive view state from `(old session asset, new transport length)`.
   v16 pins the rule: `applyTrackAudio` updates internal transport state
   synchronously so that subsequent `transport.state()` reads reflect the new
   content, but does not invoke `ITransport::Listener::onTransportStateChanged(...)`
   while the call is in flight (neither synchronously nor via deferred posting
   before return). The controller's post-apply derive picks up the new length by
   querying `transport.state()` directly. The next natural listener invocation
   (next tick, next play/pause) carries the same state and is deduplicated by the
   controller's diff.
3. **The view sources the load-target track id from `EditorViewState`.** v15
   ...
   not say where the view obtained `track_id`. v16: the view uses
...
10. **`ui::Editor` exposes a one-row thumbnail handoff.** v15 said thumbnail
    handoff "remains app/composition responsibility" but did not show how. v16:
    `Editor` exposes `[[nodiscard]] juce::Component& initialRowComponent()` and
    `void attachInitialThumbnail(std::unique_ptr<audio::Thumbnail>)`. The
    composition root calls both after construction. Multi-row dynamic thumbnail
    creation is deferred behind the `IThumbnailFactory` tripwire.
...
`length` is the duration of the currently applied audio. In v16 the editor applies
...
public:
    ...
    // (such as length) synchronously so that a subsequent
    // ITransport::state() call reflects the new content. Must NOT invoke
    // ITransport::Listener::onTransportStateChanged(...) while this call is in
    // flight - neither synchronously nor via deferred posting that runs before
    // this call returns. The controller's post-apply derive picks up the new
    // state by querying transport.state() directly. The next natural listener
    // invocation (next tick, play/pause, etc.) is allowed to carry the same
    // state and will be deduplicated by the controller.
...
`core::TrackId`, but in v16 implementations may treat applying to a second track id as
...
`load_button_enabled` is `false` when `tracks` is empty. With the v16 composition
...
- `onLoadAudioAssetRequested(track_id, asset)` first calls
  ...
  existing session track asset and pushes `last_load_error` composed by the
  controller (e.g., `"Failed to load audio asset: " + asset.path.string()`). On
  ...
  synchronously without firing a listener, the derive's `transport.state()` query
  returns the new `length`, and the controller's diff suppresses the redundant push
  that the next natural listener invocation will trigger.
...
public:
    ...
    // Returns the component whose repaint should be triggered when thumbnail
    // proxy generation completes for the editor's initial track row. The
    // composition root passes this to audio::Engine::createThumbnail(...).
    [[nodiscard]] juce::Component& initialRowComponent() noexcept;

    // Attaches a thumbnail to the editor's initial track row. Called once by
    // the composition root after construction. The row owns the thumbnail.
    // Multi-row dynamic thumbnail creation is deferred behind the
    // IThumbnailFactory tripwire.
    void attachInitialThumbnail(std::unique_ptr<audio::Thumbnail> thumbnail);
...
constructor, and exposes a fully ready root component. App code does not manually
sequence `setController` and `attachView` calls.

For v16's one-row case the composition root handles thumbnail handoff in two calls
after construction: query `initialRowComponent()`, pass it to
`audio::Engine::createThumbnail(...)`, then hand the resulting thumbnail back via
`attachInitialThumbnail(...)`. The state model is multi-track shaped so expanding to
more rows does not rewrite the controller/session contract; the dynamic thumbnail
side will land with the `IThumbnailFactory` port.
...
public:
    ...
    {
        m_editor.attachInitialThumbnail(
            m_audio_engine.createThumbnail(m_editor.initialRowComponent()));
...
`MainWindow` owns concrete objects only; it does not contain editor workflow.
...
2. **Add focused audio ports.**
   ...
   - Document the listener-suppression rule on `IPlaybackContent::applyTrackAudio(...)`.
...
3. **Adapt `audio::Engine` to the ports.**
   ...
   - Suppress transport listener invocations during apply: implement this either by
     not emitting state events from the apply code path at all, or by gating
     emission until after the apply returns; whichever is simpler in the
     Tracktion-backed implementation.
...
7. **Add `EditorController` and tests.**
   ...
   - Cover initial state push, duplicate suppression, play/pause, stop, seek, failed
     load preservation, successful load commit, track-row derivation, and the rule
     that `applyTrackAudio` does not synchronously fire transport listeners.
...
10. **Add `ui::Editor`.**
    ...
    - Expose `component()`, `initialRowComponent()`, and
      `attachInitialThumbnail(...)`.

11. **Rewire `MainWindow`.**
    - Make `MainWindow` own `audio::Engine`, `core::Session`, and `ui::Editor`.
    - Call `attachInitialThumbnail(...)` after construction using
      `initialRowComponent()` and `audio::Engine::createThumbnail(...)`.
...
TEST_CASE("EditorController: applyTrackAudio must not synchronously fire transport listeners")
{
    rock_hero::core::Session session;
...
- `Engine::applyTrackAudio(...)` does not invoke transport listeners during the call,
...
- **Automated-testable workflow:** load, play/pause, stop, seek, failed-load handling,
  cursor derivation, track-row state, and the listener-suppression rule are handled
  by `EditorController` and tested with fakes. These tests do not require JUCE
  initialization, Tracktion runtime state, an audio device, or a message loop.
...
- **Small app composition root:** `MainWindow` creates concrete engine/session/editor
  objects and relies on `ui::Editor` to keep the editor feature fully wired, with
  one explicit thumbnail handoff for the v16 one-row case.
...
- Transient inconsistent view state during loads. Addressed by the
  listener-suppression rule on `applyTrackAudio` and the controller's post-apply
  derive against `transport.state()`.
...
- `testable-modular-architecture-refactor-plan-revised-v15.md` - v15, superseded by
  this v16.
