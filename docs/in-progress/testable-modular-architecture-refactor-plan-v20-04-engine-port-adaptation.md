...
risk is the concrete adapter failing to notify transport observers consistently
after edit-driven transport changes.
...
- edit-caused length, position, or playing changes are observable through the normal
  transport listener surfaces.

If direct `Engine` construction is too heavy in this stage, extract the
transport-publication decision into a small helper that the real `Engine` uses and
test that helper. Do not satisfy the listener rule with an unrelated test-only fake
adapter; that would only test the fake. The minimum acceptable fallback is:
