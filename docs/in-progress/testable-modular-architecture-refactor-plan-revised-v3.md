# Testable Modular Architecture Refactor Plan (Revised v3)

## Purpose of This Revision

Supersedes `testable-modular-architecture-refactor-plan-revised-v2.md`. Two corrections
drove this revision, both raised by the user after v2:

1. **`audio::Thumbnail` is not a port.** v2 treated it symmetrically with `ITransport`
   ("narrow Tracktion-free audio port"). In reality, `TracktionThumbnail` holds a
   `tracktion::Engine&` supplied by `audio::Engine` and drives repaints on a JUCE
   component bound at construction. It cannot be faked, substituted, moved, or
   constructed independently of the engine. It is a *translation adapter* — it hides
   Tracktion headers from UI translation units, but it is not a seam designed for
   unit-testable substitution.
2. **`AudioAssetRef` rename.** The `Ref` suffix was redundant since the struct itself
   is the handle. Renamed to `AudioAsset` to match the bare-noun style used elsewhere
   (`Song`, `Chart`, `Arrangement`) and to stay future-proof as the type grows beyond
   a single path.

These changes cascade into the presenter's constructor (no longer takes a thumbnail),
the composition sequence (thumbnail flows engine → waveform display, not through the
presenter), and the ports inventory (only `ITransport` remains in the "port" category;
thumbnails are called out as adapters).

### Fixes Applied In This Revision

- Reclassified `audio::Thumbnail` as a translation adapter, with explicit definitions
  of "port" vs. "translation adapter" so the distinction is load-bearing.
- Removed the thumbnail from `EditorPresenter`'s constructor. Click-to-time math uses
  `TransportState.length` from the transport event — no thumbnail access needed.
- Composition: `Engine::createThumbnail(...)` now hands the thumbnail directly to the
  `WaveformDisplay`. The presenter never sees it.
- Lifetime rule stated: `audio::Thumbnail` instances must not outlive the
  `audio::Engine` that created them.
- `core::AudioAssetRef` renamed to `core::AudioAsset`.

### Intent Preserved From v2

- `juce::File` stays at the JUCE view/app edge; the port/presenter layer uses
  `core::AudioAsset`.
- `core::PlaybackPosition`, `core::PlaybackDuration` live in `rock-hero-core/timing/`.
- `audio::TransportState` lives in `rock-hero-audio/api/` and references the core time
  types.
- `ITransport` is a genuine port with a single coarse state event.
- Transport state is authoritative for view state; intents never speculatively mutate.
- `architecture.md` dependency-line update lands in the first commit that creates
  `audio -> core`.
- CMake target splits are tripwire-driven; no names pre-committed.
- Deferred ports for subsystems that don't yet exist stay deferred, each with a
  tripwire.
...
- has only one realistic implementation (the framework-backed one),
- is bound at construction to framework-owned runtime state (an engine, a component),
- is not designed for test-time substitution.
...
pull Tracktion in. But the concrete `TracktionThumbnail` holds a `tracktion::Engine&`
from `audio::Engine`, runs Tracktion's async `SmartThumbnail` caching, and repaints a
specific JUCE component bound at construction. There is no fake, no substitute, and no
unit-testing story for it.

The plan treats these asymmetrically. Ports carry the testability contract. Translation
adapters earn their existence by keeping framework headers out of UI translation units,
nothing more.
...
Does **not** carry the thumbnail handle — the view owns its thumbnail reference
independently.
...
- **`WaveformDisplay`'s `audio::Engine&` constructor parameter** and its
  ...
  takes an `audio::Thumbnail*` directly and exposes `on_seek` as
  `std::function<void(double)>` emitting a normalized x. The presenter converts
  normalized x to `core::PlaybackPosition`.
...
The thumbnail flows directly from the engine to the waveform display. The presenter
never sees it.
...
view->setPresenter(*presenter);       // wires view intents (clicks, buttons) to presenter
```

The presenter sees an `audio::ITransport&` and nothing else from the audio library.
The view owns the thumbnail. Neither sees `audio::Engine` the type — the `engine`
variable in the app is the concrete composition root and stays local to the app
target.

**Lifetime rule:** `audio::Thumbnail` instances must not outlive the `audio::Engine`
that created them. The concrete `TracktionThumbnail` holds a `tracktion::Engine&` into
the engine's owned runtime; destroying the engine invalidates every thumbnail. The
composition above enforces this naturally — destruction order is reverse of
construction, and the engine is constructed first.

---

## Ports vs. Translation Adapters: Current Inventory

### Ports (genuine boundaries, fakeable, testable)

- `rock_hero::audio::ITransport`

### Translation adapters (Tracktion-free headers, not fakeable)

- `rock_hero::audio::Thumbnail` — already exists. Hides Tracktion from the UI
  translation unit. Not a substitution seam.
...
1. **Add `core::PlaybackPosition`, `core::PlaybackDuration`, `core::AudioAsset`.
   ...
   the step where `rock-hero-audio -> rock-hero-core` becomes true.
...
3. **Rename `audio::Thumbnail::setFile(const juce::File&)` to
   ...
   `TracktionThumbnail` and all call sites.
...
7. **Rewire `main_window`** to compose `Engine` → `ITransport&` →
   `EditorPresenter` → `EditorView`, with the thumbnail routed directly from the
   engine to the view. Delete `audio::Engine::Listener`. Delete
...
- Conflating ports with translation adapters so that test fakes are expected where
  none are practical. *Addressed: inventory separates the two; thumbnail stays an
  adapter, transport is a port.*
...
- `testable-modular-architecture-refactor-plan-revised.md` — v1 revision, superseded.
- `testable-modular-architecture-refactor-plan-revised-v2.md` — v2 revision,
  superseded by this v3.
- `testable-modular-architecture-refactor-plan-review-notes.md` — feedback applied;
  artifact can be archived.
