# Testable Modular Architecture Refactor Plan (Revised v10)

## Purpose of This Revision

Supersedes `testable-modular-architecture-refactor-plan-revised-v9.md`. v10 applies four
polish fixes found in the v9 review. No architectural moves change.

### Fixes Introduced in v10
...
4. **Presenter pushes initial state on `attachSink`.** From current transport state.
...
- View sink: `attachSink(IEditorViewSink&)`. Immediately pushes a derived
  `EditorViewState` built from the current transport state (and the presenter's cached
  error field if any). This keeps the view correct when composition order attaches the
...
7. `EditorPresenter` tolerates a null sink. Its listener callback updates a cached
   ...
   flushes the cache. The view's first `setState` push therefore happens deterministically
   from `attachSink`, not from a transport event that may fire mid-construction.
...
4. **Add `ui::EditorPresenter`, `ui::EditorViewState`, `ui::IEditorViewSink` with
   ...
   caches its most recent derived `EditorViewState`, tolerates a null sink, and
   `attachSink` immediately pushes the cached state. This lands the testing story
   before any JUCE widget changes.
...
- `testable-modular-architecture-refactor-plan-revised-v9.md` — v9, superseded by
  this v10.
