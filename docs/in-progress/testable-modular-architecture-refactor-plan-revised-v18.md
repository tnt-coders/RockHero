# Testable Modular Architecture Refactor Plan (Revised v18)

## Purpose of This Revision

Supersedes `testable-modular-architecture-refactor-plan-revised-v17.md`. v18 closes
five gaps in v17: the thumbnail handoff inside `ui::Editor` is now described end to
end, the apply-time transport-listener suppression rule is broadened to cover all
state changes (not only length), the post-apply session commit invariant is named
explicitly, the `ThumbnailCreator` lifetime is pinned to construction-only, and the
`IPlaybackContent` "second track id is unspecified" comment is cross-linked to the
single-track-applied invariant. The shape from v17 stands.

### Fixes Introduced in v18

1. **Thumbnail handoff inside `ui::Editor` is fully described.** v17 said `Editor`
   creates the initial row thumbnail through a `ThumbnailCreator` callback before
   `EditorController::attachView(...)`, but did not say which `juce::Component&` was
   passed as `owner`, how the resulting `unique_ptr<audio::Thumbnail>` reached the
   `TrackWaveformRow` inside `EditorView`, or whether `EditorView` exposed an
   internal interface for that handoff. v18 commits: `Editor` forwards the
   `ThumbnailCreator` into `EditorView`'s constructor. `EditorView` constructs its
   initial `TrackWaveformRow` and then calls the callback with the row component as
   `owner`, transferring the resulting `unique_ptr<audio::Thumbnail>` into the row
...
2. **`applyTrackAudio(...)` suppresses all transport-listener events, not only
   length changes.** v17 only excluded length-change events. Today's load path also
   resets transport position to zero, which is another transport-state change a
   listener could observe against old session content if it fired synchronously.
   v18 broadens the rule: during `IPlaybackContent::applyTrackAudio(...)`, the
   engine must not invoke `ITransport::Listener::onTransportStateChanged(...)`,
   either synchronously or through a deferred post originating from work inside
   that call. The initiating controller picks up all resulting transport state
   changes via the post-apply `transport.state()` read.
3. **Post-apply session commit invariant is named.** v17 said "the session commit
   must succeed because the track was already validated," but
   `Session::replaceTrackAsset` returns `bool`. v18 makes the rule explicit:
   `EditorController` debug-asserts that `replaceTrackAsset(track_id, asset)`
   returns `true` after a successful apply. The track was just validated and
   `core::Session` is single-thread-owned, so this assert is a structural invariant
   guard; if a future `Session` change adds new failure modes, the assert
   surfaces the desync instead of silently dropping the audio/session sync.
4. **`ThumbnailCreator` is one-shot.** v18 documents that the `ThumbnailCreator`
   passed to `ui::Editor` is consumed once during construction, forwarded once to
   `EditorView`, and then discarded. Dynamic multi-row thumbnail creation is still
   deferred behind the `IThumbnailFactory` tripwire; the callback must not be
   stored as a member of `Editor`, `EditorView`, or `EditorController`.
5. **`IPlaybackContent` cross-links to the single-track-applied invariant.** v17's
   port comment said "applying to a second track id [is] unspecified behavior."
   v18 explicitly cross-references the `TransportState.length` single-track-applied
   invariant in that comment so a reader does not need to reconcile "session is
   multi-track shaped" with "second track is unspecified" without context.
...
1. **The controller validates track IDs before applying audio.** `EditorController`
   ...
   commit must succeed because the track was already validated (now debug-asserted
   per v18 fix #3).
...
`true`, the controller commits the already-validated session change (debug-asserting
that the commit succeeded) and derives a new `EditorViewState`.
...
- `onLoadAudioAssetRequested(track_id, asset)` first verifies that `track_id` exists in
  ...
  `session.replaceTrackAsset(track_id, asset)` and debug-asserts that the commit
  returned `true`. The track was just validated and `core::Session` is
  single-thread-owned, so the assert guards against future `Session` changes that add
  new failure modes; without it, a desync between audio state and session state would
  go silent. After the commit, the controller clears `last_load_error` and derives a
  new view state. Because `applyTrackAudio` updated transport state synchronously
  without firing transport listeners for any apply-caused change, the derive's
  `transport.state()` query returns the new content state.
...
- Receives an `Editor::ThumbnailCreator` callback in its constructor and uses it
  exactly once: after constructing its initial `TrackWaveformRow`, the view invokes
  the callback with the row's component as `owner` and transfers the resulting
  `unique_ptr<audio::Thumbnail>` into the row via
  `TrackWaveformRow::setThumbnail(...)`. The callback is not stored.
...
For v18's one-row case the composition root supplies concrete thumbnail creation as a
...
7. **On success**, the controller calls `session.replaceTrackAsset(track_id, asset)`,
   debug-asserts the return value is `true`, clears `last_load_error`, and derives
   a new view state. The derive reads `transport.state()`, which now reflects the
   new content because the engine updated it during step 5.
...
10. The next natural transport listener invocation (next tick, play/pause, etc.)
...
9. The `Editor::ThumbnailCreator` callback is consumed during view construction and
   is not retained by `Editor` or `EditorView`. Dynamic thumbnail creation for rows
   added at runtime is deferred behind the `IThumbnailFactory` tripwire.
...
7. **Add `EditorController` and tests.**
   ...
   - Controller validates `track_id` exists before calling
     `IPlaybackContent::applyTrackAudio(...)`.
   - Controller debug-asserts `session.replaceTrackAsset(...)` returns `true` after
     a successful apply.
...
9. **Extract `EditorView`.**
   ...
   - `EditorView` receives an `Editor::ThumbnailCreator` in its constructor; after
     constructing the initial `TrackWaveformRow`, it invokes the callback with the
     row component as `owner` and transfers the resulting `unique_ptr<audio::Thumbnail>`
     into the row.
...
- Audio/session desync from stale track IDs. Addressed by validating the track exists
  before calling `IPlaybackContent::applyTrackAudio(...)` and debug-asserting the
  post-apply session commit.
...
- `testable-modular-architecture-refactor-plan-revised-v17.md` - v17, superseded by
  this v18.
