...
| docs/roadmap/20-game-architecture-and-render-stack.md | **G20-RENDER CLOSED; Phases 1–3 complete (2026-07-10)** — Phase 4 ready | Platform scope, SDL3+bgfx spike, renderer-sharing seam, window/loop, resources, threading, dev diagnostics | Gate closed: SDL3+bgfx, loop L2, seam Option 1; Phase 1: SDL3/bgfx Conan pins + shader-compile helper (cmake/RockHeroRenderStack.cmake), GameShell L2 loop replaced the DocumentWindow shell, first game/ui Noop test target; Phase 2: resource-pack convention (GameResources resolver in game/core with the first game/core test target, resources/ deployed beside the executable, surface_flat shader pair compiled from committed .sc sources and loaded through the resolver); Phase 3: frame clock + timing instrumentation (pure FrameClock/FramePacingStats in game/core consuming plan 12's clock surface, game.frame log channels, thread map in architecture.md; engine soak transferred to plan 21 with G21 closed GO); expert checkpoints recorded in the gate record |
...
| docs/roadmap/25-note-highway-3d.md | **Phases 0–3 complete** — Phase 4 (techniques) ready | Headless highway scene model, camera, Charter-parity board/notes/techniques, feedback + debug HUD | Phase 3 done 2026-07-11: playable skeleton renders in the game (board face, beat bars, FHP runway, shadows, rails, open bars, atlas note heads with the rolling flip, glyph fret numbers/sections, overlay v1, --dev-package fixture path + dev clock); corpus chart scrolls at locked 144 fps; checkpoint fixed a Phase 2 camera depth defect; string order: lowest on top (user 2026-07-11) |
| docs/roadmap/26-game-startup-menus-library.md | Ready (Phases 5–9 behind G20-RENDER) | Package peek reader, library index cache, scan, menus, Quick Play, onboarding, previews | Not started |
