...
| **G21-TRACKTION-GO** | docs/roadmap/21-game-audio-engine-and-session.md Phase 0 | Tracktion-in-game GO confirmed (constraint (g)); signed off jointly with 20 Phase 0b criterion S1 | 21 final shell integration only — 21 Phases 1–6 run in the existing JUCE game shell before G20-RENDER closes |
...
| docs/roadmap/20-game-architecture-and-render-stack.md | **G20-RENDER CLOSED; Phases 1–2 complete (2026-07-10)** — Phases 3–4 ready | Platform scope, SDL3+bgfx spike, renderer-sharing seam, window/loop, resources, threading, dev diagnostics | Gate closed: SDL3+bgfx, loop L2, seam Option 1; Phase 1: SDL3/bgfx Conan pins + shaderc shim (cmake/RockHeroRenderStack.cmake), GameShell L2 loop replaced the DocumentWindow shell, first game/ui Noop test target; Phase 2: resource-pack convention (GameResources resolver in game/core with the first game/core test target, resources/ deployed beside the executable, surface_flat shader pair compiled from committed .sc sources and loaded through the resolver); expert checkpoints recorded in the gate record; CI Conan-cache fixes flagged for tnt-coders/ci-workflows |
...
| docs/roadmap/25-note-highway-3d.md | **Phases 0–2 complete**; Phase 3+ ready (gate closed) | Headless highway scene model, camera, Charter-parity board/notes/techniques, feedback + debug HUD | Done 2026-07-10 @ 92b95ba4: scene model + projection + camera in common/core highway/ (verticality + pin invariants tested); next = Phase 3 playable skeleton after plan 20 Phase 1 |
