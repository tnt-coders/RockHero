...
| docs/roadmap/25-note-highway-3d.md | **Phases 0–3 + look-parity pass complete** — Phase 4 (techniques) ready | Headless highway scene model, camera, Charter-parity board/notes/techniques, feedback + debug HUD | Phase 3 done 2026-07-11 (playable skeleton, locked 144 fps); look-parity pass 2026-07-11: reference camera chain (rotations + wide frustum — defect 7's zero-rotation claim corrected), Charter texture assets adopted (license corrected to BSD 3-Clause; notice ships with the assets), board-furniture parity (fret states + hit flash, lane ribbons, per-window runway, gradient beat bars, vertical shadow fan, anticipation ring, 1:1 heads, fretboard skin), renderer promoted to common/ui and shared with the editor preview |
