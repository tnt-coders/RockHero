...
- The SDL3/bgfx device-picker and calibration-wizard **UI** — that is
  docs/roadmap/26-game-startup-menus-library.md Phase 8, an SDL-presentation-only layer over the
...
- The shared per-app store type, `ActiveDeviceRoute`, `Access::ReadOnly`, and the two audio-config
  identity constants — those are docs/roadmap/13-audio-device-settings-and-calibration.md Phase 1;
  this plan **consumes** them.
- The `LiveInputMonitor` gate itself and its gate logic — that is
  docs/roadmap/14-live-input-monitoring.md (Phase 3 creates the monitor; Phase 4 wires the game's
...
- The editor's effective-source facade, "use game settings" toggle, and consolidated Audio Setup
  modal — those are docs/roadmap/48-editor-audio-setup.md; this plan is game-side only.
- Note detection / onset algorithms beyond what plan 13 Phase 4 already owns for calibration
  capture (docs/roadmap/22-note-detection.md owns detection).
- Scoring hit-window widths and the provisional-hit machine
  (docs/roadmap/24-scoring-star-power-failure.md consumes the offset contract).
...
- **docs/roadmap/26-game-startup-menus-library.md Phase 8** — the SDL device-picker and calibration
  ...
  behind SDL.
- **docs/roadmap/48-editor-audio-setup.md** — reads the game's audio-config file read-only through
  the shared store's `activeDeviceRoute().identity`; this plan mirrors the primary player's route
  into that field (P1).
- **docs/roadmap/24-scoring-star-power-failure.md** — consumes the effective-offset contract that
