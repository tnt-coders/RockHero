...
- History: docs/completed/plugin-window-transport-key-routing-plan.md records why the native hook
...
- **Undo/redo never yield to plugin text fields; Space yields when the plugin's focused control
  wants it** — docs/completed/plugin-window-transport-key-routing-plan.md; preserved unchanged.
...
- **Phase 4 is the risky one** (Win32 hook + VST3 focus behavior, historically fragile per
  docs/completed/plugin-window-transport-key-routing-plan.md). The uninjected built-in-trio
