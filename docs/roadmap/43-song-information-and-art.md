...
- `docs/todo/audio-asset-catalog-thumbnail-cache-plan.md` is about *waveform* thumbnails for audio
...
(`docs/in-progress/tone-track-tempo-map-plan.md` is active work); this plan's editor-core phases
...
- Art undo mementos hold file bytes in memory; if repeated art swaps ever make history memory a
  ...
  restore (constraint (f); `docs/completed/editor-undo/editor-undo-plan.md`).
