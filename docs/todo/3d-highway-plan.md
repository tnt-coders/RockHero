...
without re-deriving the source-game analysis.

## Goal

A source-game-feeling 3D note highway for the game executable: the player looks down a
...
- **the source game** defines the feel: readable at speed, calm camera, the note "lands" on a
...
- **[Charter](https://github.com/Lordszynencja/Charter)** (MIT) is the implementation reference.
  ...
  the source game feel and its geometry/camera decisions are worth adopting; its implementation has
...
## source-game Analysis: How Charter's 3D View Works
...
- **Y = string axis.** Board surface ("chartboard") at y=0; strings stacked upward with
  ...
  the source game's default puts the low string at the bottom of the screen.
...
## the source game Comparison (what to keep, add, or skip)

Charter reproduces most of the source game's functional language: string-colored heads with technique
...
- **the source game's camera** cuts between framings and glides more cinematically; Charter's single
...
- **the source game's venue** (animated stage, lighting, crowd) is atmosphere we replace with a cheap
  parallax background layer initially (Charter's `/4` parallax trick), leaving venue rendering
  as a separate future feature.
- **the source game renders misses/ghosts** and gameplay scoring feedback; that belongs to the
  detection/scoring system, not this plan — but the renderer must expose hooks (hit, miss,
  anticipation state come from outside).
- the source game fades/dims already-passed notes quickly; Charter's fading shader does the same.
...
- Bend-curved rails with per-bend-point interpolation, outer-string bend inversion, prebend
  offset at the head; bend amount labels only if readability testing wants them (the source game
  doesn't label in 3D).
...
- Slide rails with Charter's easing curves (pitched `sin³`, unpitched reversed sine) — these
  match the source game's read well; unpitched slides additionally dim toward the end.
...
- Whether chord fingering panels show by default (the source game shows them contextually).
