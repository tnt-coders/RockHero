...
| the source game import format | open-source-package (Conan) | source-package format read/write for the source game packages |
...
- `libs/rock-hero-core` remains framework-free: no JUCE, no Tracktion. It may use private
  format-specific Conan packages such as `libzip`, `nlohmann_json`, and `open-source-package`.
...
| MIT | open-source-package | Permissive, compatible with everything |
