...
| Song format | open-source-package (Conan) | source-package format read/write for song packages |
...
- `libs/song` depends on standard C++ only; no JUCE, no Tracktion. May use format-specific Conan packages (e.g. `open-source-package`).
...
| open-source-package | MIT | Permissive, compatible with everything |
