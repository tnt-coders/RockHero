...
| **E26** | A **dead predecessor justifies no connection** — neither the hammer nor the pull motion; a `LeftTap` after a dead note stays legal | User, 2026-08-20 (walkthrough W14). A hammer-on or pull-off carries the predecessor's energy into the next note, and a deadened string has none to carry. The legitimate "hammer from nowhere" after a dead note is the fretting hand's own strike, which is exactly what `LeftTap` records — and the resolver answers a `LeftTap` locally, so it needs no exemption. Asked of the `dead` flag alone: a palm-muted predecessor rings and hands over as any note does; a both-muted note already sounds as dead. The converse (E24, a claim ON a dead note from a ringing predecessor — the muted hammer) is untouched. **Relational, and a resolver clause from the day it was ruled**: `resolveLegato` disqualifies a dead predecessor beside the fret-hand harmonic, so a claim after one reads as the plain pick it plays as and the settle sweep flattens it at the next settle point — the normalization the ruling asked for, with no new mechanism. Deadening a predecessor therefore breaks the claim after it, and the dead-note toggle window round-trips that exactly as every technique window does (the second X reverses before any settle could flatten; across a settle the flatten folds into the X entry, one undo restoring both). |
...
(its fret is picking-hand travel — same physics as E19); a fully-muted predecessor CANNOT — **revised 2026-08-20 as E26**; the earlier reading ("its finger
is a real press, and releasing it is the muted pull") mistook the press for the energy, and a
deadened string hands nothing over, so the strike after a dead note is the `LeftTap`; "which note is the predecessor" is
...
|---|---|---|
| E5 | `Pull` requires a same-string predecessor whose **released** fret is higher — the last pitched waypoint's, or a scrape's slide-out's. A scrape predecessor is VALID data (D7: executable with gain) that the `H` verb never *creates*, because the assist refuses to reshape a gesture carrier's tail; its organic authoring route is pull-first-then-scrape. A fret-hand-harmonic predecessor is disqualified outright (E19), and so is a dead one (E26); a palm-muted predecessor is ordinary. The predecessor must also be **still holdable at the pull's onset** (D13): past the kept-sustain bound (`g_minimum_kept_sustain_beats`) a disconnected tail is a proven release. | **resolver clause** (`resolveLegato`, judging `releasedFret` + `predecessorHoldReaches` against the span-extended hold; one authority for the surfaces, the gameplay build, the reader, and the `H` planner) |
...
| E26 | No connection FROM a dead note (either motion — a deadened string has no energy to hand over); the `LeftTap` after one stands because it reads no predecessor | **resolver clause** (the predecessor disqualification, on the `dead` flag alone) |
