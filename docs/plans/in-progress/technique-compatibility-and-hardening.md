...
| `held` | `optional<int>` — the finger the fretting hand plants behind the stop a picking-hand onset sounds, legal only where that hand stops the string (`pickingHandStopsString`: a plain `Tap` or a `PickSlide`, no node) |
...
| **E21** | `harmonic_node` > the **physical stop**, strict — `physicalStopFret`: the note's own `fret`, or the capo when that fret is 0 | **Enforced** (`chart_rules.cpp`). A node lies on the speaking length, so nothing vibrates at or behind the stop; a node *at* the stop is the stop. Generalized by D1: under the 0-means-open convention a capo'd open string stores `fret = 0`, so the stop the node must clear is derived, not stored. ONE field on every arm, RULED 2026-09-17: `fret` is the stop the string speaks from whichever hand stopped it, so a tapped harmonic states its pressed stop there exactly as an artificial one does, and the rule never asks which hand made the onset. |
...
follows worth stating: a *fretted* tap harmonic is representable, since the fret-hand bound (E22)
binds only the family whose own finger stands on the node — and RULED 2026-09-17 that fretted form
is the family's ONE shape. `fret` is the stop the string speaks from, so an artificial and a tapped
harmonic alike hold the pressed stop there under the touched node, and `fret == 0` is the
open-string case of that same shape rather than a second reading of the field.
...
| ~~**H2**~~ | **PROMOTED to E13** — a fret-hand harmonic does **not** require `Pick` | A tapped harmonic — tapping directly over the node — is a real technique, so `Tap` + a node is playable. This is the asymmetry with E3 and the reason the two harmonic kinds cannot share one rule. The pairing E13 names is the open-string one; a tapped harmonic over a PRESSED stop states that stop in `fret`, the one shape the whole harmonic family uses (ruled 2026-09-17). |
...
- ~~**Q5** a fret-hand harmonic + the legato motions~~ — **ANSWERED.** `Pull` forbidden on every
  ...
  test rejected the open-string tap harmonic, and retires the third-kind plan. Two consequences
  above.
