...
| **E5** | A pull-off needs a preceding note on the same string at a **higher** released fret, **still holdable at the onset** | Something must be released to sound it — and still held to release. The hold half (D13): past the kept-sustain bound (`minimumKeptSustainBeats`) a held predecessor necessarily carries a ring reaching the minimum-sustain-distance margin, so a shorter one is a proven release (`predecessorHoldReaches`). **Not a validation rule:** it is the resolver's Pull clause, so a claim it refuses reads as a plain pick rather than refusing the document. |
