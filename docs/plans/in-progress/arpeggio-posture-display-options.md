...
Status: **SETTLED.** The table below under "What shipped" records the decision that closed this
option space; the live rule is stated in code and pointed at from there. Everything else here is
the option space the decision was chosen from, kept because the dead ends are expensive to re-walk
and several were rejected for reasons no measurement would have found.

**Two of the rejected options have lost their premise.** Hand-posture spans and their postures are
derived from the notes instead of stored, and a posture carries neither a name nor a fingering —
nothing ever authored either, so the view state's shape name, its per-string finger, the timeline
ruler's name-chip band, and the highway's fingering panel are all gone as branches that could never
fire. Option A's name chip and option B's fingering-bearing diagram would therefore have to bring
their own data source with them: when names and fingerings are authored they arrive as a dictionary
keyed by a posture.
...
**The SLOT does not ask which hand made the head.** A centred digit sits exactly where a head sits
and the note pass paints after the brackets, so any head sounding elsewhere covers it and the
satellite is the only slot that survives; the comparison is on PLACES, not on printed numbers, so
two facts that happen to print the same digit are still two facts. The hand answers a different
question — WHO prints the displaced digit — which is the fourth row above. The live statement of
both is `ShapeStringViewState::digit`'s doc block in `chart_view_state.h`, the one authority the
painter and the hit target both read, and it is deliberately not restated here.

Two changes to the tail came with this mark, both of which fix problems wider than it: the tail's
**fill** is the linked-note fill (as dark as the keyframe heads riding it) while its **edge** stays
at full brightness, and the tail has no right **end cap**. The bracket marks read off that
surviving tail edge rather than the note fill, since a fill colour chosen against a bright tail
...
(`highway_renderer.cpp`, the shape-rail constants and the chord/arpeggio box pass), positioned *at
the frets* because the highway's space IS the fretboard. It prints no posture digits anywhere and
does not need to.
...
1. The mark must read as **belonging to the posture** — the brackets are the posture mark, so a
   number outside them reads as detached.
...
4. It must work at ~**8 px** cap height, where the lane's fret font hits its floor.
...
the whole reason this took four rendering rounds.
...
- **No product displays per-string posture inline during an arpeggio.** MuseScore's own issue
  ...
  convention offered. This is genuinely new ground.
- **But the information is not new; its *form* is.** The universal 2D representation of a
  ...
  fingering, neither of which our derived posture stores.
...
- **State-on-change is the lead-sheet convention**: a chord symbol persists until a new one is
  ...
  "a hand position being established … and nothing else".
...
the same tab projection that would supply names.
...
- **Known flaw — VERIFIED, and it is worse than "inherits the suppression".** The ruler runs greedy
  left-to-right overlap suppression for every label row (`RulerRowPlacement` in
  `timeline_ruler.cpp`): a label occupies its measured width plus `g_label_width_pad{8}`, and the
  next may not start until `g_pinned_label_gap{10}` (in `sticky_label.h`, beside the pin law that
  spends the same clearance) past it. The class's own comment records that "on dense maps far more
  candidates arrive per rebuild than survive suppression." So the flaw is **self-defeating, not
  merely inherited**: a chip carrying `Dm x-0-3-2-1-x` is roughly four times the width of one
  carrying `Dm`, and a wider chip suppresses proportionally more of its neighbours. The more posture
  the chip carries, the less often it is drawn — and it fails first exactly in the dense tapping
  material this whole case comes from.
...
- **Buys:** it is *the* idiom for this information, so it needs no learning; it is spatial, which
  means it says the same thing the 3D box says, in the same way; it is the only option that could
  show fingering at all.
- **Costs:** needs real vertical space — a diagram is not a chip; the label band would have to grow
  ...
  And the fingering it exists to carry is not stored: it needs the posture-keyed dictionary first.
...
- **Note:** the hold-to-peek variant is this option with a while-held key instead of a toggle. **A
  while-held key cannot be a registry command** (commands fire on press, and no `keyPressed`
  decoder owns key-state since total rebindability), so peek would be the one non-rebindable
  binding in the editor. A toggle avoids that entirely. If peek is still wanted later, the ghost
  vocabulary already exists to render it.
...
### I — Darken the sustain tail — attacks the cause, not the symptom

Multiply the tail's fill and edge down from `base x0.66`. The sighting instrument cycled 1.0 / 0.75
/ 0.55 / 0.40 independently of the posture candidate, so any posture candidate could be judged
against any tail.
...
- **It converges the surfaces**, which no other option here does. The 3D highway's tails are
  already more translucent, so this moves 2D toward 3D rather than further from it — the direction
  the no-divergence law wants.
...
## Test harness — the method, recorded rather than kept
...
grammar), because neither is a contrast problem. The harness is not kept — it is throwaway
scaffolding, and it costs a mutable module-level variant in `tab_paint_core.cpp` (a shape this
codebase otherwise avoids, tolerated only for an experiment with one UI-thread writer), an enum and
two accessors in the header, a switch in the bracket pass, an `EditorCommandId` with its
registration, its perform arm, and a row in the locked keybind table. Rebuild it the same way if
another candidate ever needs sighting.

The cycle was a rebindable command in the Authoring category, like anything else. Each press
advances one candidate and repaints; nothing is persisted, no undo entry is written, and the lane
returns to the shipped candidate on restart. The cycle deliberately isolates *causes* as well as
candidates:

| # | Candidate | What it isolates |
|---|---|---|
| 1 | Satellite, cased | the baseline that was rejected in live use |
...
information".** The insight is that the collision sits on the axis with no room — strings are fixed
— while **time has slack**. So the digit keeps the centred-in-brackets reading that works, and when
a head occupies the span start the whole bracket *slides past it* on the same string: `o [ 3 ]`.
The head keeps its own centre, the posture keeps its own enclosure, nothing is hidden, and no new
register, casing, colour or size is introduced. Candidate 8 adds the only suppression that is
honest — a head sounding the posture's OWN place is a true duplicate, so that one string's digit is
dropped and every non-duplicate still shows.
