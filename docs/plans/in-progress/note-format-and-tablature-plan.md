...
[Charter](https://github.com/Lordszynencja/Charter) is a BSD-3-Clause-licensed source-format chart
...
- **One physical onset = one note event; no link-next flag.** Decision 2026-07-05.
  Charter and the source format model tied notes as separate events joined by `linkNext`; 26 Charter files
...
- **Linking is an editor command over payload storage, never a stored flag.** Decision 2026-07-06:
  ...
  link machinery, the source-format checker tooling, and Guitar Pro's gesture semantics — and upheld.
...
- The existing playback-follow behavior is unchanged. Once the tab renders over the waveform,
  ...
  `docs/plans/todo/smooth-scroll-follow-evaluation.md` (settled trigger).
...
- **Notes say what sounds; shapes say what the hand holds; templates are reusable postures.**
  ...
  and arpeggios with a single mechanism (source corpus data confirmed chords and handshapes are ~1:1
...
- **Sections.** Navigation/practice markers; the type vocabulary starts small and grows as
  needed (no commitment to Charter's 31 kinds; the source format's name+number pairs collapse to type,
...
- Owns the deferred chart-storage questions (grid tokens, chord modeling, technique
  ...
  `docs/plans/todo/chart-note-storage-future-work.md`, which was removed 2026-07-05 when this plan
  absorbed it.
- Feeds `docs/plans/todo/arrangement-difficulty-derivation-plan.md`: the format deliberately carries
