...
- **[D2] The span walk is keyframe-blind** (verified: zero keyframe reads in chart_shapes.cpp).
  Hold a chord, slide one finger — the held shape changes mid-span and no rule says whether that
  splits, reclassifies, or does nothing; the box keeps claiming the start shape while a member
  audibly leaves it, the one breach of posture constancy. Options: (a) member fret travel splits
  the span at the departure — LAW III's own answer, and it makes sounding holds change by
  keyframes exactly where silent holds change by restatement; (b) it reclassifies; (c) the box
  states only the ONSET shape by definition (document the limit). Recommendation: (a).
