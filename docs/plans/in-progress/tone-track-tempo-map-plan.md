...
This plan builds on `docs/plans/completed/tempo-map-implementation-plan.md`, a completed prerequisite: it
...
Earlier parallel plans were reconciled 2026-07-05: `docs/plans/todo/tone-track-plan.md` was removed
(its remaining ideas contradicted the settled RackType, musical-position, and Tracktion-managed
persistence decisions), and `docs/plans/todo/tone-automation-track-plan.md` was revised down to the
...
- The synthesized default region is runtime-only: it is **not** written back to `song.json`. A
  ...
  projects (see `docs/plans/todo/native-package-write-safety-followups.md`).
