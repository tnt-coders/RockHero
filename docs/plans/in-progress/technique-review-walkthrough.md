...
- [x] **W9-H — Is the scrape toggle a true ON/OFF? RULED: YES.** *"pressing Shift+X a second time
  ...
  joining the family is one enumerator and one law row. The scrape arms on the entering AND the
...
- [x] **D13 — The release-inference refinement of legato.** The sustain conventions make "the
  ...
  - **The two data facts.** (1) The import drop rule sheds the tail of any effect-free note notated
    shorter than the kept-sustain bound — the shared `g_minimum_kept_sustain_whole_note`,
    meter-scaled through `minimumKeptSustainBeats` (`grid_arithmetic.h`), so the drop rule and the
    inference can never disagree. (2) A held tail is trimmed/clamped to end exactly the
    minimum-sustain-distance margin (`g_minimum_sustain_distance_whole_note`, meter-scaled) before
    the next same-string onset, so "ring end reaches (next onset − margin)" IS the maximum
    representable hold.
