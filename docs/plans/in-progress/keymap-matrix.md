...
| Keybind | Verb | Status |
|---|---|---|
| `H` | **legato** — one verb for hammer-on and pull-off; the direction derives from the predecessor on that string. Three things gate it beyond the frets: the judged fret is the predecessor's **released** one (its last waypoint, or a scrape's slide-out end), the predecessor must still be **holdable** at this onset (past the kept-sustain bound a disconnected tail is a proven release), and a fret-hand-harmonic predecessor is disqualified outright. Then the released fret picks the direction — higher = pull, lower = hammer — and an ambiguous case (no predecessor, an equal fret) is refused rather than guessed. When the hold test is the only blocker, the verb authors the connection itself (the D14 assist): the plan grows the predecessor’s tail to the margin point and derives the direction in the same undo entry, bounded by the duration verb’s growth clamp. While the selection and history still prove the previous press was this verb’s own, a second press reverses it exactly — grown tails included — and drops the entry (ruling 4’s true toggle); otherwise the press means apply-or-clear on the eligible subset | **Live** (`planSetLegato` + `ChartLegatoToggle`, default chord plain `H` — verified in `editor_command_registry.cpp`). Verified against the official GP8 manual's shortcut appendix: Guitar Pro binds `H` to "Hammer On / Pull Off" the same way. Its `Shift+H` "Legato" is a sheet-music slur that does not change the hammer/pull state, so it has nothing to map onto here and stays unbound |
| `Ctrl+H` | **force hammer-on** — the **sole author of the left-hand tap**, the one statement the fret relationship cannot infer, valid across E4's domain (positive sounding position) and able to override a derived pull. `Ctrl` means *precision* — "I will state it exactly, do not infer" — and there is deliberately no force-Pull, pulling off to a higher fret being physically impossible | **Live** (`ChartForceHammer`, default chord `Ctrl+H` — verified in `editor_command_registry.cpp`). The verb is `planSetAttack(Hammer)`: its written-form validity check yields the ruled domain from the one rule authority — the no-node open string is the sole skip, an open-string pinch's bridge-side graze refuses to re-hand, and a tap harmonic's strike point carries into E13's hammer form |
...
- **A chord for the pick-slide toggle.** The verb ships registered and enabled with no default,
  ...
  the force-hammer verb (registered that day), so the scrape needs its own chord rather than a
  variant of `H`.
