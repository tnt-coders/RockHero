...
| Keybind | Chart (highway) | Automation lanes | Tone strip | Status |
|---|---|---|---|---|
| `0`–`9` / numpad `0`–`9` | type **fret** at armed caret, or onto the selection — notes and the arpeggio bracket of a selected hold marker alike (2026-08-27) | open **value editor** at armed caret | `✗` | Live |
...
| `[` / `]` | **free** — grid moved to `+/-` | `—` |
| `L` | claimed by **legato** since the 2026-08-12 technique-letter amendment — the link/slide reservation fulfilled (see the technique verb table; `Shift+L` there carries the same verb extended with travel, its waypoint-disconnect clause live since 2026-08-26) | Live |
...
| `W` | reserved for the **whammy bar** (future feature; user 2026-08-13) — unbound. Recorded alternative: `Shift+V` may take whammy instead if wide vibrato is dropped or vibrato strength becomes tunable data, freeing that chord — `Shift+V` fits the sibling grammar, `W` has the mnemonic and the user likes it; the official call waits for the whammy work | `—` |
...
| `N` | **arpeggio hold (note-type conversion)** — promote a note to a silently-held member of the earlier shape ("my finger was already there"); the underivable fact the bracket needs to state a shape from its initial onset when a member is not struck until later. Design + storage: `docs/plans/todo/arpeggio-authoring.md` | **SIGNED 2026-08-25, Live 2026-08-26** (`ChartHoldMarkerToggle`, labelled "Arpeggio Hold"; verb `planToggleHoldMarker`; user: *"what if … we used 'N' … I guess N could stand for something like 'Note Type'? … This is a common enough thing that I like having a bare key for it, not Shift modified"*). `N` verified unclaimed in this matrix and the registry, no reservation. Supersedes the same-day `Shift+A` signing after the user's frequency judgment — a bare key for a common charting verb outranks the corpus proxy, which measures imports rather than authoring. The registry's `Shift+A` technique-block comment took its matching one-line correction in the same change. Unlike every other row here the verb is CARET-anchored rather than selection-scoped: it acts on whatever the armed slot holds (empty → a fret-less hold marker, a note → converted into a marker carrying its fret, a marker → removed), because a fact about the fretting hand is authored at the position it holds. It shares the technique verbs' toggle window, so a second press reverses the first entry exactly — the only thing that can restore a converted note's ring and techniques. **Sighted 2026-08-27, chord unchanged**: the verb draws no mark of its own — the arpeggio bracket at the span start IS what a marker looks like and selects as — and a selected bracket takes a typed fret through the ordinary digit row below |
