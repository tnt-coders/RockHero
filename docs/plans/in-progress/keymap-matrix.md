...
1. Chart pointer drag-editing — **CLOSE: drag-move only** (reposition a note by mouse, on the placement quantum; the `Ctrl` off-grid tier is retired). **Sustain edge-drag dropped** — `Alt`+wheel is already the mouse sustain command, so an edge-drag would be redundant. · **scheduled**
