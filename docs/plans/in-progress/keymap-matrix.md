...
| `H` | **natural harmonic** — freed by legato's move; the strongest first-letter mnemonic in the map (GP's `Y` is legacy). A GP `H` habit now authors a loud, visible, undoable wrong mark instead of the silent off-by-one link it authored before — an improvement even for the habit it breaks | reserved (verb unbuilt: the typed fret cannot name the node, because one fret range can hold two nodes — 3.16 and 3.86, 2.31 and 2.67 — so the verb presents the nodes within the typed fret's range as a list and the charter picks; a picker surface, built beside bend authoring) |
| `Shift+H` | **pinch harmonic** — the natural harmonic's sibling | reserved (verb unbuilt; lands with `H`) |
