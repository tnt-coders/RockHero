...
| `Ctrl+P` | `Ctrl+Shift+P` (step 4b) | position marker (fret-hand position) | arrangement, chart | grid slot | this gate's ruling | **RESERVED** behind the FHP gate |
| `Ctrl+H` | `Ctrl+Shift+H` (step 4c) | span marker (what the hand does) | arrangement, chart | grid slot | this gate's ruling | **RESERVED** behind the FHP gate (whether span and position are one object or two is its first ruling; one object means one chord) |
