...
> in.
>
> **Amended 2026-09-11 (user-signed): on the chart lane, bare means a NOTE and `Alt` means the
> PATH.** One sentence decides every entry row below. On the keyboard the modifier says which: a
> bare digit and bare `Insert` author a NOTE at the armed slot, `Alt`+digit and `Alt`+`Insert`
> author on the PATH already running there. At the pointer `Alt` is what makes a gesture author at
> all, so the press COUNT says which: `Alt`+click states on the path, `Alt`+double-click authors the
> note. Where a ring covers the slot, a note gesture SPLITS that note losslessly — the original ends
> exactly at the new head, the new note opens in the state the hand holds, and every keyframe after
> the split rides it — so NOTHING SINGLE-PRESS TRUNCATES A RING OR CLIPS A KEYFRAME. `Alt` is
> unchanged in meaning: it is still the authoring gate, and stating a point on an existing path is
> the authoring act. The rows below carry the split; `TypePathDigit0`–`9` ("Type Path Digit N") and
> `InsertPoint` ("Insert Point") are the new commands, and `NeutralInsert` is renamed "Insert Note"
> beside them.
>
> *Later the same day, superseding the morning's first form of this amendment:* that form had a
> bare gesture on a covered slot STRIKE a head and TRUNCATE the ring under it, a re-strike the
> existing gate settled. The truncating strike is gone from every entry gesture, replaced by the
> lossless split above. The ring clamp and the clearance repair still exist — for load, for import,
> and for the MOVE verb, which is the one editing gesture that re-strikes by truncation: a landing
> inside a tail SHORTENS that ring and rides its release back to the new end, deliberately, since a
> moved note brings its own payload and there is nothing coherent to merge. It never DELETES a
...
| Keybind | Chart (highway) | Automation lanes | Tone strip | Status |
|---|---|---|---|---|
| `0`–`9` / numpad `0`–`9` | **A NOTE at the armed caret** — type a **fret** and author an onset there (re-ruled 2026-09-11: bare means a NOTE, `Alt` means the PATH; the `Alt` row below states on the path instead, and this row's earlier same-day form, which struck a head through the ring and TRUNCATED it, is superseded). On an EMPTY slot, a head. At the EXACT END of a ring, a head — the ring already stops there, so there is nothing to divide and nothing to shorten, which is what makes sequential entry safe: a digit at the previous note's end is always the next note (where a slide-out already ends on that slot, arming the caret selects its chip and the digit RETYPES the fall; the strike onto that slot is `Insert` or `Alt`+double-click, which place the head and let the gate ride the release back one margin, its ring shortening with it — the one single-press placement that moves a keyframe, clipping nothing). STRICTLY INSIDE a ring, a **LOSSLESS SPLIT** of that note at the caret's instant, the new head carrying the typed fret. Refused over an existing head. **The split is the disconnect verb's segment walk** — `planDisconnectKeyframes` generalized to any covered instant, with the attack as a parameter: legato where `Shift+L` disconnects, a pick here — so one walk serves both and there is no second rule to keep in step. The original note ends exactly at the new head; the NEW note opens in the state the hand holds — the stated fret in force (here the typed one), a bend in force as its onset bend, a shake in force opening it shaking; every keyframe after the split rides the new note, a slide-out included; a keyframe sitting exactly AT the split becomes the new head; a glide cut mid-leg leaves the first note holding its stated fret while the new note travels on to the arrival; and the first note's arrival retreats one margin before the new head, exactly as the disconnect already does. NOTHING SINGLE-PRESS TRUNCATES A RING OR CLIPS A KEYFRAME: the ring clamp and the clearance repair remain the authorities for load, for import, and for the MOVE verb alone. Multi-digit through the shared 750 ms window; an illegal fret paints red and discards. A digit over a non-empty SELECTION retypes it instead — heads and the arpeggio bracket of a selected silently-held stop alike (2026-08-27) — whichever verb it carries, bare or under `Alt`: a non-empty selection is the operand either way, and only a digit at a bare caret has a verb to choose between. Which STOP the digits state is the caret's channel (2026-08-27, the held-fret increment): bare digits state the note's own sounding fret, and digits after `N` — or after clicking the satellite digit beside a bracket, or after stepping the caret onto that satellite — state the fretting hand's `held` stop under a right-hand onset. One pending entry either way; the channel decides where it lands and where its red box draws. A selected KEYFRAME retypes the same way (W13 ruled, 2026-09-09): the flow, the multi-digit window and `planRetypeFrets` are the note flow's, the entry simply gained a keyframe operand, and the SELECTION KIND — not a third `ChartStopChannel` value — says which stop the digit reached. Its pending BOX has no target yet, since the view publishes retype targets as note indices; a refused keyframe digit is therefore silent until that display lands (`docs/tracking/backlog.md`). A digit CONTINUES a live pending entry rather than opening a second one, and the FIRST digit's verb is that entry's: a digit of the other verb settles the live entry before beginning its own | open **value editor** at armed caret | `✗` | Live |
| `Alt`+digit · `Alt`+numpad digit | **A POINT on the PATH at the armed caret** — state the typed fret on the path already running there rather than authoring a note at it (new 2026-09-11; this is the meaning the bare digit used to carry on a tail). STRICTLY INSIDE a ring, a KEYFRAME on that note stating the typed fret. At the EXACT END of the ring, that point IS the slide-out, the release keyframe — the one keystroke on the lane that authors a fall, which is exactly what the bare digit vacated there by becoming the next note. On an EMPTY slot the convenience head, exactly what the bare digit would have placed, so a mistimed `Alt` costs nothing. Nothing over a keyframe already at the slot. The multi-digit window, the red-box refusal and the retype-the-selection clause are all the digit row's above — one pending entry, whichever verb opened it. Registered on BOTH key codes because `Alt` changes which one arrives: under `Alt` a numpad digit reaches JUCE with the TOP-ROW code on Windows, so the top-row chord is the one that matches there (`TypePathDigit0`–`9`, "Type Path Digit N") | `✗` | `✗` | Live |
...
| `Insert` | **A NOTE with no digit to type** — the digit row's verb with the fret supplied rather than typed. On an EMPTY slot, and at the EXACT END of a ring, a **fret-0** head. STRICTLY INSIDE a ring, the same **LOSSLESS SPLIT** the digit row states in full, the new head opening on the **RUNNING fret** — the fret the path holds at that instant — rather than on fret 0, because the split's own rule is that the new note opens in the state the hand holds. Refused over an existing head; nothing here truncates the ring or clips a keyframe. Named **"Insert Note"** in the registry (`NeutralInsert`), now that an "Insert Point" stands beside it | on-curve point at caret | `✗` (no keyboard) | Live |
| `Shift`+`Insert` | **THE SAME NOTE, AT THE FRET IN FORCE** — `Insert` with one default changed and nothing else about the gesture touched (new 2026-09-11). **THE FRET IN FORCE** on a string at a slot is the fretting hand's position handed forward by the LAST onset on that string BEFORE the slot: a picked or legato note hands forward where its ring LEFT the hand — the position channel in force at its ring's end INCLUDING the release, so a glide hands forward the fret it travelled TO and a slide-out the fret it FELL TOWARD (re-ruled 2026-09-11: the fall is travel the hand really takes, so it is where the hand ends up); a RIGHT-HAND onset — a tap, a scrape — hands forward its HELD stop instead, since its own fret is the picking hand's, read as the RESOLVED claim so a stop a pull-off derives counts exactly as an authored one, and 0 where it holds nothing; a fret-hand harmonic hands forward its stop; a silently-held stop hands forward its own fret, which is the whole of what it states. Where nothing precedes on the string, 0 — the bare default, which is what makes the modifier harmless at the start of a lane. So on an EMPTY slot, and at the EXACT END of a ring, the head lands on that fret instead of on fret 0; STRICTLY INSIDE a ring this is the bare `Insert` exactly, since the split's new head already opens on the RUNNING fret, which IS the fret in force. ONE authority for the definition — `fretInForceOn`, beside `chartPathTailAt` in the editor's chart-edit layer — which the `Shift+Alt`+click row below calls too. Named **"Insert Note, Repeating Fret"** in the registry (`NeutralInsertRepeat`) | same as `Insert` — a lane point always states a value, so there is no fret-less default to repeat | `✗` (no keyboard) | Live |
| `Alt`+`Insert` | **A POINT on the PATH with no fret to type — a SILENT one** on the note whose ring covers the armed slot, restating the fret the path already holds there — the last fret STATED at or before the caret's offset, the note's own where nothing states one earlier, never the interpolated travel — selected, with the caret armed on it, so the digit that follows gives the point its fret and the technique keys address it as any keyframe. That is the STRICTLY INSIDE case alone. At the EXACT END of the ring, and on an EMPTY slot, a fret-less statement would have nothing to say — a fall to the fret already in force says nothing — so the key places the fret-0 head there instead, and authoring the slide-out stays `Alt`+digit's, the one gesture that brings a fret with it (re-ruled 2026-09-11: bare means a NOTE and `Alt` means the PATH, replacing the one `Insert` that used to branch on what lay under the caret; the same day's first form, which had this key author the release at a ring's end, is superseded). Over a keyframe already at the slot it does nothing. **THE COMMIT LAW: a silent point is AUTHORING STATE** (re-ruled 2026-09-10): a point that says nothing — no bend, no shake, a fret the path passes through anyway — pushes NO undo entry (the history records written states, and it writes as nothing), dissolves when its NOTE leaves focus (no entry either, and nothing on the history stack can defer it), collapses before undo or redo replays, and is never written (the document writer and the load repair both shed it). So a charter places the point first, walks the tail to where the slide lands, and gives the start its meaning second — and the one entry the landing pushes carries both points. Every refusal is the rule authority's through the finalize gate — offset zero, past the ring, onto an existing point, a path a harmonic or open string may not carry, the capo floor, a scrape a repeated position would still refuse | `—` (a lane point always states a value, so there is no fret-less form to insert) | `✗` (no keyboard) | Live (`InsertPoint`, "Insert Point") |
...
| **`Alt`+click** | **A POINT on the PATH** at the pointer — the keyboard `Alt`+`Insert`'s pointer form exactly. STRICTLY INSIDE a ring, a SILENT point on that note restating the fret the path holds there, selected with the caret armed on it so the next digit gives it its fret. At the ring's EXACT END, and on an EMPTY slot, the fret-0 note, since a fret-less statement would say nothing there. Over a keyframe already at the slot, nothing but the caret arming there, as today | insert on-curve point | **split** region | Live |
| **`Shift+Alt`+click** | **THE SAME POINT VERB, ITS HEAD AT THE FRET IN FORCE** — `Alt`+click with the one default the `Shift`+`Insert` row defines above, and nothing else about the gesture changed (new 2026-09-11). STRICTLY INSIDE a ring it IS `Alt`+click: the silent point restating the fret the path holds, selected with the caret armed on it — there is no fret-less head there to default, so the modifier has nothing to say. At the ring's EXACT END, and on an EMPTY slot, the head it places takes the fret already in force on that string instead of fret 0. Over a keyframe already at the slot, nothing but the caret arming there, as the bare gesture does | insert on-curve point (the modifier says nothing off the chart) | **split** region | Live |
| **`Alt`+double-click** | **A NOTE** at the pointer — the pointer form of `Insert`, `Alt` being what makes a pointer gesture author at all, so on this device the press COUNT is what says note or path. STRICTLY INSIDE a ring, the same LOSSLESS SPLIT, its new head opening on the RUNNING fret; at the ring's EXACT END, and on an EMPTY slot, the fret-0 head. It needs no case of its own: the first press states a silent point, which is authoring state, and the second dissolves it and authors the note through it — exactly what replacing authoring state with a written onset means | `✗` | `✗` | Live |
...
head refuses the key outright — you change a note by retyping its fret). The chart's `Insert`
STRICTLY INSIDE a ring is not a second exception: it divides that note LOSSLESSLY, the original
ending exactly at the new head and everything it was still saying — its later keyframes, the bend
and the shake in force — riding on into the new note. Nothing is overwritten and nothing is clipped:
a ring is testimony to what one strike is still sounding rather than a target, and dividing a
statement in two destroys none of it. A plugin is the one object
...
column holds two products rather than one specialization, because bare means a NOTE and `Alt` means
the PATH: a bare digit and `Insert` author a note at the slot, `Alt`+digit, `Alt`+`Insert` and
`Alt`+click state a point on the path already running there. That is the same row shape, not a
divergence — the other surfaces have no running path to join, so their single verb is the note one
...
- **F — DECIDED: one named exception + create framing.** "Insert never mutates an existing object"
  ...
  **Amended 2026-09-11:** the chart's `Insert` authors a NOTE, and strictly inside a ring it does so
  by SPLITTING that note losslessly — the original ends at the new head, everything it was still
  saying rides into the new note, and nothing is truncated or clipped. That stays inside the rule:
  the key refuses over a head, and dividing a ring conserves every statement the note made instead
  of overwriting one. The filled plugin slot remains the one named exception.
