...
| `Insert` | fret-0 note at an armed EMPTY slot; **on any ringing tail: a keyframe planted for real** at the fret the path holds there — the last fret STATED at or before the caret's offset, the note's own where nothing states one earlier, never the interpolated travel — selected, with the caret still on its slot, so the digits and technique keys that follow address it as any keyframe (re-ruled 2026-09-09: no ghost, no window, every tail not only a path-carrying one; a note INSIDE a tail is `Alt`+click's, since a keystroke that chops a ring is the more surprising meaning). **THE COMMIT LAW is the settle's**: a point that still says nothing when its NOTE leaves focus — no bend, no shake, a fret the path passes through anyway — dissolves at that settle, its entry retired, so a charter places the point first and gives it a meaning second. Focus is the lane's reveal (re-ruled 2026-09-10): the note selected, a point of it selected, or the caret inside its ring — the moment its real tail stops showing — so the caret may walk the tail from the slide's start to its landing and type the landing fret without the start dissolving underfoot. Every refusal is the rule authority's through the finalize gate — offset zero, past the ring, onto an existing point, a path a harmonic or open string may not carry, the capo floor, a scrape a repeated position would still | on-curve point at caret | `✗` (no keyboard) | Live |
