...
|---|---|---|
| `Ctrl+M` | read against the MEASURE the cursor is in, never against what is selected: a free downbeat adds a section and prompts for its name; an occupied one SELECTS the section standing there, and a second press reopens that section's prompt to rename it. The select press is the keyboard's way onto a chip, which is what lets `Enter` rename and `Delete` remove one with no mouse (the marker grammar's restate; `Ctrl+M` is the section's letter on the document plane, and `Ctrl+S` is save) | Live (signed 2026-09-12) |
