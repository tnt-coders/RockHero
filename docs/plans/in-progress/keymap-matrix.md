...
|---|---|---|
| `Ctrl+M` | with no section at hand, add one at the cursor's measure downbeat, name from a prompt; where a section IS at hand — the selected chip, or one already standing on that downbeat — reopen that section's prompt on its own name to rename it (the marker grammar's restate; `Ctrl+M` is the section's letter on the document plane, and `Ctrl+S` is save) | Live (signed 2026-09-12) |
