...
- **A chord for the pick-slide toggle.** The verb ships registered and enabled with no default,
  ...
  the row above and nothing else in the registry uses it, so the scrape needs its own chord rather
  than a variant of `H`.
