...
- **Rules** (`chart/chart_rules.h`): `validateChartRules(chart, tempo_map)` enforces the
  ...
  (open question recorded in `docs/in-progress/note-format-and-tablature-plan.md`).
...
Restated from `docs/in-progress/note-format-and-tablature-plan.md` (the source for all
...
7. **Undo is the RockHero-owned editor-core history** (constraint (f);
   `docs/completed/editor-undo` records the settled design; the shipped mechanism is
