...
(docs/plans/roadmap/26-game-startup-menus-library.md), and a local corpus-calibration harness. Structural
...
- No difficulty/intensity computation — docs/plans/roadmap/11-derived-difficulty-calculator.md.
...
- Consumed by:
  - docs/plans/roadmap/40-chart-editing.md — live-feedback hooks call the Phase 3/4 entry points; the
  ...
  - docs/plans/roadmap/28-practice-mode.md — motivates the "no sections" sanity finding (section loops).
...
- The structural rule set is corpus-validated and stays a hard read gate
  (docs/plans/in-progress/note-format-and-tablature-plan.md, "Corpus validation (2026-07-06)").
- Shapes are advisory notation; "validation can advise when sounding notes under a span disagree
  with the template posture" (docs/plans/in-progress/note-format-and-tablature-plan.md, Format
  Specification commentary).
- Same-string sustain-overlap semantics were deliberately left open until the endpoint
  arithmetic exists (docs/plans/in-progress/note-format-and-tablature-plan.md, "Open Questions").
- Difficulty is derived, never authored; one true tab per arrangement
  ...
  docs/plans/in-progress/note-format-and-tablature-plan.md, "Deliberately absent").
...
- Scope: `lint/arrangement_lint.{h,cpp}` — `lintArrangement(const Arrangement&, const TempoMap&)`
  ...
  versioning discipline docs/plans/roadmap/10-format-versioning-and-chart-identity.md defines for
  format changes.
- Public-header impact: one new public header.
...
- The one standing risk is scope creep from lint into the read gate: promoting any rule to a
  ...
  docs/plans/roadmap/10-format-versioning-and-chart-identity.md with a corpus impact analysis — never do
  it inside this plan.
