...
until cleared. Game-facing looping UX (practice mode) is docs/plans/roadmap/28-practice-mode.md, not this
plan; both products drive the one backend this plan lands.

## 3. Non-goals

- No playback speed factor. `setPlaybackSpeed` is docs/plans/roadmap/21-game-audio-engine-and-session.md
  Phase 1 (typed-NotSupported) and docs/plans/roadmap/28-practice-mode.md Phase 1 (functional).
...
- No keybind centralization. Any key this plan ships lands in the existing scattered handling and
  migrates under docs/plans/roadmap/46-editor-keybinds.md (stated explicitly in Phase 3).
- No IPlaybackClock work. The editor reads transport on the message thread today and does not
  depend on docs/plans/roadmap/12-playback-clock.md; that plan's extrapolator loop-wrap snap rules apply
...
- docs/plans/roadmap/21-game-audio-engine-and-session.md — Phase 1 declares the same shared-port loop
...
- docs/plans/roadmap/28-practice-mode.md — Deferred; its Phase 2 hardens game-side wrap behavior and its
...
- docs/plans/roadmap/12-playback-clock.md — no dependency either way. Its extrapolator already defines
...
- docs/plans/roadmap/40-chart-editing.md — Phase 3 will add glyph-select/marquee pointer input in the tab
  ...
  Phase 2.
- docs/plans/roadmap/46-editor-keybinds.md — any key this plan ships (Escape clear, optional Ctrl+L)
...
1. **Whichever-executes-first coordination rule (stated here, binding on plans 21/28/47).** The
   ...
   docs/plans/roadmap/21-game-audio-engine-and-session.md Phase 1 or THIS plan's Phase 1 executes first
...
3. **The editor loop selection is stored as musical positions** (a `common::core::GridPosition`
   ...
   docs/plans/roadmap/41-tempo-map-authoring.md lands tempo editing, a loop pinned to measures 17–21
...
6. **Playback follow on wrap is already settled**: the shifted-window follow snaps when the
   ...
   docs/plans/todo/smooth-scroll-follow-evaluation.md's record). A loop wrap is exactly that case when
...
9. **The editor 3D preview is a second consumer of the same loop state, never a second loop**
   ...
   docs/plans/roadmap/28-practice-mode.md §7). Phase 2's `LoopSelectionViewState` is THE editor loop:
   the preview (docs/plans/roadmap/44) renders it from `EditorViewState` and modifies it only by
...
1. **Q1 — Drag surface for the time selection.** Options: (A) the timeline ruler band —
   ...
   click-to-seek and stays free for docs/plans/roadmap/40-chart-editing.md Phase 3's future
   ...
   no longer available: `docs/plans/in-progress/editing-interaction-model.md` (settled 2026-07-09)
...
- **Scope**: headless selection model in editor-core:
  ...
  - Snap helper: `nearestTempoGridPosition(...)` in editor-core
    ...
    pixel-perfect freedom. Coordination: if docs/plans/roadmap/40-chart-editing.md Phase 1 has landed
...
- **Scope**: interaction on `TimelineRuler` plus rendering:
  ...
  - Clearing: Escape in `EditorView::keyPressed` clears the selection; click-away semantics per
    ...
    docs/plans/roadmap/46-editor-keybinds.md Phase 1 executes — this plan must not build any registry.
...
- **Scope**: connect selection to the Phase 1 port and verify the wrap-adjacent behaviors:
  ...
  - **Verify-before-implementing checkpoint (tone across wraps)**: the expert findings prove
    ...
    docs/plans/roadmap/21-game-audio-engine-and-session.md Phase 3c(2) — extend its wrap matrix if it
  ...
  - **Playback follow on wrap**: no code — decision 6; manual soak confirms off-screen-left snap
    ...
    (docs/plans/todo/smooth-scroll-follow-evaluation.md) stays the user's pending decision; this plan
...
docs/plans/roadmap/00-roadmap.md status line updated, and plans 21/28 annotated that the shared loop
