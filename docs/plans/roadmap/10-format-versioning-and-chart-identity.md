...
- A canonical **chart-identity hash** — computed from a semantic serialization of chart content +
  ...
  `docs/plans/roadmap/29-online-leaderboards.md` scores against, `docs/plans/roadmap/26-game-startup-menus-library.md`
  caches on, and `docs/plans/roadmap/24-scoring-star-power-failure.md` stamps into score records.
...
- No new `song.json` metadata fields (album art, sort fields, preview start/length) — those are
  `docs/plans/roadmap/43-song-information-and-art.md`, which routes its format changes through this plan.
- No serialized difficulty — `docs/plans/roadmap/11-derived-difficulty-calculator.md` owns derivation and
...
- No changes to chart *content* semantics; the format spec stays as recorded in
  `docs/plans/in-progress/note-format-and-tablature-plan.md` (reference only — active work).
...
- (d) **Derived over authored**: the chart hash is derived from content; no authored "chart
  ...
  `docs/plans/roadmap/43-song-information-and-art.md`, which recommends resolving chart "version" via this
...
- `docs/plans/roadmap/43-song-information-and-art.md` — its metadata fields are the first real bump; it
  must not start format changes before Phase 2 lands the ladder.
- `docs/plans/roadmap/29-online-leaderboards.md` — keys submissions on the Phase 3 hash + algorithm id.
- `docs/plans/roadmap/26-game-startup-menus-library.md` — library cache keys include the Phase 3 hash;
  its malformed-package listing consumes Phase 2's typed newer-format error.
- `docs/plans/roadmap/24-scoring-star-power-failure.md` — score record format embeds the Phase 3 hash and
  algorithm id.
- `docs/plans/roadmap/11-derived-difficulty-calculator.md` — cache entries key on (chart hash, calculator
...
- **Difficulty and other descriptors are derived, never authored** —
  ...
  not authored data") and `docs/plans/roadmap/11-derived-difficulty-calculator.md`. Nothing in this
...
- **Chart positions are exact rational grid tokens, never seconds/floats** —
  `docs/plans/in-progress/note-format-and-tablature-plan.md` "Format Specification (v2)"; `Fraction`
...
Absorbs the still-valid core of `docs/plans/todo/native-package-write-safety-followups.md`, re-verified
2026-07-06: the narrowed tone-reference write-safety work is complete
(`docs/plans/completed/native-package-write-safety-discussion.md`), but the archive itself is still
...
- **Phase 2** keeps v1 parsing byte-identical (empty ladder); the risky surface is the public
  ...
  the first real step ships with `docs/plans/roadmap/43-song-information-and-art.md` where its content
  change lives.
