...
- The game-side calibration wizard UI and device-setup screens — those ship with
  docs/plans/roadmap/26-game-startup-menus-library.md; this plan lands the headless architecture only.
- Note detection or onset algorithms beyond the minimal self-contained cue detector needed for
  calibration capture (docs/plans/roadmap/22-note-detection.md owns detection).
...
- docs/plans/roadmap/24-scoring-star-power-failure.md — consumes the effective-offset contract from
  Phase 3 (measured offsets shift hit-window comparisons; calibration offsets are recorded in the
  score record format).
- docs/plans/roadmap/26-game-startup-menus-library.md — ships the calibration wizard UI and device-setup
...
- docs/plans/roadmap/21-game-audio-engine-and-session.md — the game session restores the shared device
  state from Phase 1's store at startup; live-monitoring latency stance here informs its per-tone
  latency policy.
- docs/plans/roadmap/12-playback-clock.md — offsets are applied by consumers (scoring, render lead) in the
  clock's audio-sample time domain; they are never baked into the clock itself.
- docs/plans/roadmap/27-in-song-flow-results-profiles.md — device-loss auto-pause interacts with its
  pause/resume pre-roll flow and non-destructive score state.
- docs/plans/roadmap/22-note-detection.md — the capture flow's cue-response detector is deliberately
  self-contained at v1; it may later delegate to 22's onset detector without contract changes.
- docs/plans/roadmap/20-game-architecture-and-render-stack.md — vsync/frame-pacing policy (recorded there)
...
- **Calibration is per physical input route** (backend + device + channel index; channel display
  ...
  docs/plans/completed/per-device-input-calibration-plan.md.
...
- **Effective-offset contract** (the normative rule docs/plans/roadmap/24-scoring-star-power-failure.md
  ...
  - a detection event timestamped `t_in` in input-stream sample time corresponds to player
    ...
    clock domain (docs/plans/roadmap/12-playback-clock.md); hit windows are centered on chart time and
  ...
  - the highway render leads the clock by `video_offset_ms` (consumed via
    docs/plans/roadmap/20-game-architecture-and-render-stack.md's render loop and
...
wizard (docs/plans/roadmap/26-game-startup-menus-library.md) and any editor/dev harness drive the same
code.
...
- On success the workflow writes `LatencyOffsets` (with the measurement snapshot) through
  ...
  (docs/plans/completed/per-device-input-calibration-plan.md) remains a separate, already-shipped
...
- **Policy** (assumes open question 5's recommendation A):
  ...
  - game contract (implemented by docs/plans/roadmap/26-game-startup-menus-library.md and
    docs/plans/roadmap/27-in-song-flow-results-profiles.md): freeze score/session state
...
docs/plans/todo/audio-device-settings-extraction-followups.md relevant at this boundary: a
