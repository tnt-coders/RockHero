...
docs/plans/roadmap/23-detection-verification-harness.md, defined in Phase 0a below) and on the
...
- **No gameplay scoring code exists anywhere.** `rg -i score` over first-party sources matches
  ...
  docs/plans/roadmap/24-scoring-star-power-failure.md and do not exist yet.
...
- No doc under `docs/plans/todo/` covers leaderboards or networking (verified by listing and grepping
...
- docs/plans/roadmap/10-format-versioning-and-chart-identity.md — the semantic chart-identity hash phase:
...
- docs/plans/roadmap/22-note-detection.md — the accuracy-metric definition phase: G29-STABILITY is
  expressed in the metrics that plan defines (per-technique precision/recall, latency
  distribution).
- docs/plans/roadmap/23-detection-verification-harness.md — the harness measures G29-STABILITY; its
  serialized DetectionEvent replay and autoplay bot generate deterministic test submissions for
  Phase 2's backend harness.
- docs/plans/roadmap/24-scoring-star-power-failure.md — the score-record-format phase (per-note verdict
...
- docs/plans/roadmap/27-in-song-flow-results-profiles.md — the local score store keyed
...
- docs/plans/roadmap/26-game-startup-menus-library.md — the menu input layer and the results/library UI
  surfaces where board views render.
- docs/plans/roadmap/00-roadmap.md — carries gate G29-STABILITY and the AGPL/licensing decision; this
...
- Boards key on the semantic chart-identity hash with a versioned hash-algorithm id — never a
  byte-level package hash (docs/plans/roadmap/10-format-versioning-and-chart-identity.md).
...
- Friends-scale integrity scope (heuristics, not hard anti-cheat) is recorded here as the
  normative scope and mirrored in docs/plans/roadmap/00-roadmap.md.

## 8. Open questions for the user

Each is mirrored into docs/plans/roadmap/00-roadmap.md § Decisions needed. None block writing this plan;
all block Phase 1.
...
docs/plans/roadmap/23-detection-verification-harness.md CI reports. The numbers below are this plan's
proposed defaults; reconcile them against the metric definitions plan 22 actually ships and
record the reconciled values in docs/plans/roadmap/00-roadmap.md as the gate:
...
- Verification: if Q4=A the service builds via the standard helper
  ...
  with the backend and is registered in docs/plans/roadmap/00-roadmap.md — either way the game tree
...
- Offline-first submission queue: persisted per profile, retries with backoff, survives restarts;
  ...
  opt-in on the results screen (docs/plans/roadmap/27-in-song-flow-results-profiles.md flow), with a
  remembered "always submit" preference.
- Board views on the results screen and library song list
  (docs/plans/roadmap/26-game-startup-menus-library.md surfaces), navigable via the menu input layer.
...
docs/plans/roadmap/00-roadmap.md.

## 11. Rollback/abort notes

- **Phase 0 may conclude "do not build."** That is a valid outcome (cost, licensing burden, or
  stability never converging); record it in docs/plans/roadmap/00-roadmap.md and stop. Nothing upstream
