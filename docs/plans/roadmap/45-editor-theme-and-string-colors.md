...
(`docs/plans/roadmap/44-editor-3d-preview.md`), and the game note highway
(`docs/plans/roadmap/25-note-highway-3d.md`) draw from one source of truth. The plan also owns the
...
- Game-side settings UI for preset selection — the registry this plan builds is product-neutral;
  the game persists its choice through `docs/plans/roadmap/27-in-song-flow-results-profiles.md`'s
  IGameSettings and surfaces the picker via `docs/plans/roadmap/26-game-startup-menus-library.md`.
- Any 3D rendering — consumers are `docs/plans/roadmap/25-note-highway-3d.md` and
...
- Note-detection support for 9–10 string instruments — raising the display/authoring cap does not
  promise detectability; see `docs/plans/roadmap/22-note-detection.md` (its latency budget flags sub-B0
...
- The shared palette header in common must not include any JUCE header. It is plain data plus
  ...
  `docs/plans/roadmap/20-game-architecture-and-render-stack.md`) can consume it without dragging
...
- **Doc/code mismatch** — `tab_view.h:45–49` documents the tertiary tier as "teal, magenta,
  ...
  `docs/plans/in-progress/note-format-and-tablature-plan.md` ("String count..." section, dated
...
- Phase 5 coordination (not code dependencies, but gate inputs):
  - `docs/plans/roadmap/22-note-detection.md` — its detection contract notes that 9–10 string low
  ...
  - `docs/plans/roadmap/10-format-versioning-and-chart-identity.md` — confirm against its bump rules that
...
- Downstream consumers (they depend on this plan):
  - `docs/plans/roadmap/25-note-highway-3d.md` — consumes Phase 1 (shared palette + derivation); it
    defers all note/string color definitions here.
  - `docs/plans/roadmap/44-editor-3d-preview.md` — consumes Phase 1 and the Phase 3 preset registry.
  - `docs/plans/roadmap/40-chart-editing.md` — defers the `g_max_chart_strings` raise to Phase 5 here.
  - `docs/plans/roadmap/26-game-startup-menus-library.md` /
    `docs/plans/roadmap/27-in-song-flow-results-profiles.md` — game-side preset picker and persistence of
...
1. **EditorTheme stays editor/ui-private**; it is the single seam for app-chrome colors, read at
   ...
   palette *data*, not the theme (`docs/plans/roadmap/25-note-highway-3d.md`, Current-state notes: "the
...
7. **Settings stay per-product**: EditorSettings is editor-specific by design and the game gets
   its own IGameSettings port (`docs/plans/roadmap/27-in-song-flow-results-profiles.md`). Both products
...
Mirror all four into `docs/plans/roadmap/00-roadmap.md` Decisions-needed.
...
4. **When to raise `g_max_chart_strings` to 10.** (A) Execute Phase 5 immediately after Phase 4
   ...
   **Recommendation: A**, with the explicit note that display/authoring support does not promise
   detection support (`docs/plans/roadmap/22-note-detection.md`).
...
- New feature folder `string_colors/`:
  ...
  - Charter-exact `darkerColor`/`brighterColor`/`multiplyColor` (ported integer math from
    ...
    `StringStyle`), so `docs/plans/roadmap/25-note-highway-3d.md` derives identical surfaces;
...
(`docs/plans/roadmap/23-detection-verification-harness.md` fixture generators respect the constant
