...
- No navigation, session, or run-integrity changes — the shared-navigation decision
  (docs/plans/roadmap/28-practice-mode.md §7) already makes those display-agnostic; this plan swaps
  presentation only.
- No new notation features (e.g. section-marker rendering) — docs/plans/roadmap/40-chart-editing.md
...
- No feedback/scoring semantics: docs/plans/roadmap/24 and docs/plans/roadmap/25 Phase 5 own the event feed
...
- docs/plans/roadmap/28-practice-mode.md §7 — the shared-navigation decision this plan consumes:
  intents, capability sets, sessions, and run integrity are display-agnostic; the 2D view swaps
  only presentation.
- docs/plans/roadmap/40-chart-editing.md — coordination, not a gate: its Phase 3 adds pointer input,
...
- docs/plans/roadmap/24-scoring-star-power-failure.md (event feed) and docs/plans/roadmap/25-note-highway-3d.md
  Phase 5 (pure feedback-state reduction, event log → per-note visual state) — Phase 6 only.
- docs/plans/roadmap/26-game-startup-menus-library.md Phase 4 / docs/plans/roadmap/27-in-song-flow-results-profiles.md
  Phase 1 — the settings surface where 30-Q3's display-mode choice lives.
- docs/plans/roadmap/44-editor-3d-preview.md — the preview window Phase 5 extends with the toggle.
...
- Navigation, capability sets, sessions, and score-record integrity are consumed unchanged from
  docs/plans/roadmap/28-practice-mode.md §7 / docs/plans/roadmap/24 §6 / docs/plans/roadmap/27 §6.
...
All three ANSWERED 2026-07-11 (mirrored in docs/plans/roadmap/00-roadmap.md "Decisions needed");
...
2. **30-Q2 — sequencing vs plan 40.** Options were: (a) extract the paint core **before**
   docs/plans/roadmap/40-chart-editing.md Phase 3 adds tab-lane interaction — `TabView` is verified
...
- **Scope**: the three-way display mode in `rock-hero-game/ui` per §7 — 3D highway, 2D tab, or
  **both composed in one frame** (2D strip docked as a band, 3D highway filling the rest;
  ...
  docs/plans/roadmap/28-practice-mode.md §7; in "both", the single clock/loop/feedback state drives
...
- **Scope**: `PreviewSurface` hosts the highway renderer, the tab-strip renderer, or both on the
  ...
  view-state snapshot and navigation-intent wiring (docs/plans/roadmap/44-editor-3d-preview.md §7).
