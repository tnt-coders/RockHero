...
applies it (docs/plans/roadmap/20), and the menu binding map to Phase 5, which defines the action
...
- The GameplaySession spine and tone switching — docs/plans/roadmap/21-game-audio-engine-and-session.md.
- The render-stack decision, resource-pack conventions, and the vsync/frame-pacing policy —
  docs/plans/roadmap/20-game-architecture-and-render-stack.md records those; this plan consumes them.
- Computing difficulty/intensity values — docs/plans/roadmap/11-derived-difficulty-calculator.md; this
...
- Adding album-art, sort, or preview fields to song.json —
  docs/plans/roadmap/43-song-information-and-art.md owns those format changes (routed through
  docs/plans/roadmap/10-format-versioning-and-chart-identity.md).
- Creating the MIDI infrastructure — docs/plans/roadmap/24-scoring-star-power-failure.md creates the
  `IMidiTrigger` port; this plan is a second consumer.
- Editor keybind centralization — docs/plans/roadmap/46-editor-keybinds.md.
...
- docs/plans/roadmap/00-roadmap.md — execution order and gate registry.
- docs/plans/roadmap/20-game-architecture-and-render-stack.md Phase 0a–0c — HARD GATE for Phases 5–9:
...
- docs/plans/roadmap/10-format-versioning-and-chart-identity.md — unknown-field tolerance policy for the
...
- docs/plans/roadmap/11-derived-difficulty-calculator.md — calculator-versioned intensities stored in the
  index (Phase 2 fields); until it lands, every intensity is the "Unknown" bucket defined here.
- docs/plans/roadmap/13-audio-device-settings-and-calibration.md — shared device settings store and
...
- docs/plans/roadmap/22-note-detection.md — tuner screen consumed by Phase 8 step 3.
- docs/plans/roadmap/24-scoring-star-power-failure.md — `IMidiTrigger` port and MIDI device
  enumeration/persistence; Phase 5's pedal input source shares it.
- docs/plans/roadmap/27-in-song-flow-results-profiles.md — `IGameSettings` port phase; this plan's Phase 4
  consumes it. The roadmap should order that single phase ahead of Phase 4 here.
- docs/plans/roadmap/42-chart-validation.md — richer content-validation reasons surfaced in the library
  warning view (soft dependency; Phase 7 shows package-level errors regardless).
- docs/plans/roadmap/43-song-information-and-art.md — sort fields, album art (Phase 3 album-art images, Phase 7
  columns), preview start/length (Phase 9).
- docs/plans/roadmap/21-game-audio-engine-and-session.md — preview snippet playback path (Phase 9).
...
- Difficulty/intensity is derived by versioned calculators, never authored
  ...
  docs/plans/roadmap/11-derived-difficulty-calculator.md, which absorbed the former todo derivation
  plan).
...
- The game UI follows a fixed-reference-resolution + uniform-scale model, not JUCE's
  logical-pixel + global-scale-factor model (docs/plans/todo/editor-ui-scale.md "Out of scope").
...
4. **Bindable-action sharing with the editor** (mirrored in docs/plans/roadmap/46-editor-keybinds.md,
...
5. **Bundled starter song**: onboarding ends at a "suggested first song", but a fresh install has
   ...
   resource (dual-use as a docs/plans/roadmap/23-detection-verification-harness.md CI fixture),
...
- **Scope**: new API `readRockSongPackageDescription(package_path)` returning a
  ...
  docs/plans/roadmap/10-format-versioning-and-chart-identity.md can replace it with the
...
- **Scope**: the first real game code. `LibraryIndex` value model: entries keyed by absolute
  ...
  docs/plans/roadmap/10-format-versioning-and-chart-identity.md lands — enables move detection and stable
  ...
  until docs/plans/roadmap/43-song-information-and-art.md adds art), scan status (Ok | Warning with typed
...
Depends on the `IGameSettings` port phase of docs/plans/roadmap/27-in-song-flow-results-profiles.md
...
- **Scope**: settings fields this plan owns — `first_run_completed`; library scan roots (list of
  ...
  docs/plans/roadmap/20-game-architecture-and-render-stack.md (this plan persists and displays; 20's
...
- **Scope**: bindable menu actions (`MenuAction`: up/down/left/right, accept, back, pause-menu,
  ...
  docs/plans/roadmap/46-editor-keybinds.md semantics without sharing code — open question 4). Input
  ...
  via the `IMidiTrigger` port created by docs/plans/roadmap/24-scoring-star-power-failure.md — the pedal
...
- **Scope**: menu tree — Main (Quick Play, Options, Rescan Library, Quit); Options hosts video
  ...
  (docs/plans/roadmap/13-audio-device-settings-and-calibration.md), bindings (Phase 5), and re-run
  ...
  docs/plans/roadmap/43-song-information-and-art.md sort fields when present (raw metadata fallback
...
- **Scope**: when 43's optional preview start/length fields exist, highlighting a song plays that
  ...
  docs/plans/roadmap/21-game-audio-engine-and-session.md — no second decode stack (constraints (e), (g)
