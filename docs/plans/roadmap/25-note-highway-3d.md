...
- No note detection, scoring, or gameplay-rule logic — hit/miss/early/late events are *inputs*
  to the renderer, produced per docs/plans/roadmap/24-scoring-star-power-failure.md.
...
- No editor integration — the editor keeps the 2D tab lane; docs/plans/roadmap/44-editor-3d-preview.md
...
- No menu, library, or settings UI — the lefty-mirror and scroll-speed *flags* land here; their
  user-facing toggles ship with docs/plans/roadmap/26-game-startup-menus-library.md and the settings
  store of docs/plans/roadmap/27-in-song-flow-results-profiles.md.
...
- Chart sidecar naming is canonical: `charts/<uuid>.chart.json`
  ...
  on read — validation is docs/plans/roadmap/10-format-versioning-and-chart-identity.md scope, not ours.
...
- docs/plans/roadmap/20-game-architecture-and-render-stack.md — **hard gate**: Phase 0a (platform
...
- docs/plans/roadmap/12-playback-clock.md — `IPlaybackClock` and the game-side extrapolation policy; the
...
- docs/plans/roadmap/21-game-audio-engine-and-session.md — the GameplaySession that actually plays a
  song while the highway scrolls; required for the milestone-0 vertical slice, not for this
  plan's unit-tested core phases.
- docs/plans/roadmap/45-editor-theme-and-string-colors.md — the shared string-color palette definition
...
- docs/plans/roadmap/24-scoring-star-power-failure.md — hit/miss/early/late and provisional-hit events
...
- docs/plans/roadmap/22-note-detection.md (via 24) — detection confidence values shown in the debug
  overlay.
- Reverse dependencies: docs/plans/roadmap/44-editor-3d-preview.md consumes the headless scene model and
  camera per plan 20 Phase 0c; docs/plans/roadmap/26-game-startup-menus-library.md and
  docs/plans/roadmap/27-in-song-flow-results-profiles.md surface the lefty and scroll-speed settings
...
3. **Scroll speed / visibility window as a player setting, and its interaction with
   ...
   store; no coupling to docs/plans/roadmap/11-derived-difficulty-calculator.md output at v1.
...
- Scope: confirm plan 20 Phase 0a–0c sign-off; record the chosen scene-model library (below,
  ...
  plan's Current state inventory against the then-current tree per docs/plans/roadmap/00-roadmap.md
  rules.
...
- Before starting: work through the bgfx handle-ownership watch item (wrap every retained handle
  in UniqueBgfxHandle, mind the shutdown-ordering trap), which was gated on this phase. (Resolved
  during Phase 3/4 and now retired — see `docs/tracking/watch-items.md` § Retired.)
...
- Scope: consume plan 24's event stream (hit / miss / early / late, provisional-hit lifecycle,
  ...
  shared common/ui widget set from docs/plans/roadmap/28-practice-mode.md §7's shared-navigation
...
- Testing: feedback state reduction (event log → per-note visual state) is pure and lands in
  ...
  consumer: docs/plans/roadmap/30-game-2d-tab-view.md Phase 6 presents the same reduction in the 2D
