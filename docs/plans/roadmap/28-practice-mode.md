...
This plan's single NOW requirement is delegated: docs/plans/roadmap/21-game-audio-engine-and-session.md
Phase 1 and docs/plans/roadmap/12-playback-clock.md Phases 1–4 must carry a playback speed factor and
...
- No editor-side practice features (the editor already has free seek and will get its own
  preview via docs/plans/roadmap/44-editor-3d-preview.md).
...
- (b) Public-header minimalism; ports-and-adapters per docs/design/architectural-principles.md
  ...
  existing transport port surface from docs/plans/roadmap/21-game-audio-engine-and-session.md Phase 1.
...
- (i) Real guitar input: both hands are on the guitar mid-loop. Loop restart, speed nudge, and
  ...
  (docs/plans/roadmap/24-scoring-star-power-failure.md IMidiTrigger) — keyboard is a fallback, never the
  only path.
...
- Test targets that later phases extend: `rock_hero_common_audio_tests`
  ...
  docs/plans/roadmap/21-game-audio-engine-and-session.md Phase 2 / docs/plans/roadmap/27 Phase 1.
...
- docs/plans/roadmap/21-game-audio-engine-and-session.md — Phase 0 (Tracktion-in-game GO), Phase 1 (the
...
- docs/plans/roadmap/47-editor-loop-selection.md — the expected first implementer of the Tracktion-backed
  loop range on the shared transport port (whichever-executes-first rule in that plan's
  Decisions, shared with docs/plans/roadmap/21-game-audio-engine-and-session.md Phase 1). Once its
...
- docs/plans/roadmap/12-playback-clock.md — Phases 1–3 (snapshot carries `playback_rate`), Phase 4
  (extrapolator snaps on loop-wrap backward jumps). Practice at 50% must not produce a smoothed
  clock gliding through the wrap.
- docs/plans/roadmap/24-scoring-star-power-failure.md — per-note verdict log and score-record format with
...
- docs/plans/roadmap/27-in-song-flow-results-profiles.md — Phase 1 (IGameSettings, profile id), Phase 4
...
- docs/plans/roadmap/23-detection-verification-harness.md — Phase 1 (serialized DetectionEvent replay)
  and Phase 2 (autoplay bot) for deterministic practice-loop tests without audio.
- docs/plans/roadmap/26-game-startup-menus-library.md — Phase 5 (menu input layer and bindable actions)
  for the practice UI entry points and controls.
- docs/plans/roadmap/20-game-architecture-and-render-stack.md — Phase 0 (render stack sign-off) and
...
- docs/plans/roadmap/25-note-highway-3d.md — highway renders correctly at non-1.0 `playback_rate` from
  the clock (no highway change should be needed if it consumes the clock; verified in Phase 5).
- docs/plans/roadmap/22-note-detection.md — no direct dependency for stretching (detection taps the live
...
- External decision: time-stretch backend choice and licensing sign-off (open question 1;
  coordinate with the roadmap's licensing audit — docs/plans/roadmap/00-roadmap.md Decisions needed).
...
- Speed factor and loop-region seek are plumbed through the shared transport port and the clock
  ...
  docs/plans/roadmap/21-game-audio-engine-and-session.md Phase 1 and docs/plans/roadmap/12-playback-clock.md
...
- **Navigation is a shared, display-agnostic surface; sessions differ, the vocabulary does not**
  ...
  (docs/plans/roadmap/44 §7, docs/plans/roadmap/47 §7) — same intents, math, and widgets, different
  ...
  (docs/plans/roadmap/27-in-song-flow-results-profiles.md §6); richer navigation is practice-only,
  and run integrity is enforced in the score record (docs/plans/roadmap/24 §6), never by trusting
  hidden UI. The surface is display-agnostic by design: a future 2D game view
  (docs/plans/todo/game-2d-tab-view.md) swaps only the presentation behind the same intents,
...
3. **What practice runs record.** Options: (A) accuracy-only, nothing persisted; (B) full verdict
   ...
   docs/plans/roadmap/24-scoring-star-power-failure.md's record format either way.
4. **Hit-window domain at reduced speed.** At 50% speed a window defined in song time doubles in
   ...
   wall-clock width. **Recommendation: A**, co-owned with docs/plans/roadmap/24's hit-window spec; note
   that detection confirmation latency (50–80 ms worst case per docs/plans/roadmap/22-note-detection.md)
...
5. **Loop pre-roll.** The player must re-place the fretting hand at each wrap. Options: (A) fixed
   ...
   the pre-roll plays a count-in click (asset from docs/plans/roadmap/20 Phase 2's resource pack).
...
- Verification checkpoints (each cites or closes an expert open uncertainty; run with
  ...
  b. Whole-clip slow-down route: clip `setSpeedRatio` + `setAutoTempo(false)` vs `autoTempo` with
     ...
     advance scales by the factor — this is the semantics docs/plans/roadmap/12 assumes
...
Scope: upgrade docs/plans/roadmap/21 Phase 1's `setPlaybackSpeed` from typed-error-on-non-1.0 to
...
jump → extrapolator snap), tone regions re-apply at the wrap target (docs/plans/roadmap/21 Phase 3 test
matrix already covers wrap — extend it, don't duplicate), and wrap-to-pre-roll positioning is a
plain `seek` so no new engine concept is needed. docs/plans/roadmap/47-editor-loop-selection.md Phase 1
...
Only if neither that plan nor docs/plans/roadmap/21 Phase 1 landed a functional loop (21's rollback note
...
rows (docs/plans/roadmap/27 Phase 4), section-sanity validation (docs/plans/roadmap/42-chart-validation.md),
...
Exit criteria: tests green; docs/plans/roadmap/27's results computation compiles against the shared
...
Scope: `rock-hero-game/core` headless orchestration over GameplaySession (docs/plans/roadmap/21 Phase 2):
...
Pre-roll
notes flagged unscored. Each loop iteration closes a verdict-log slice (docs/plans/roadmap/24 format).
No-fail is unconditional in practice.

Public-header impact: new `rock-hero-game/core` headers only (game-internal). Testing
(`rock_hero_game_core_tests`): deterministic replay per docs/plans/roadmap/23 Phase 1 — scripted
...
Scope: per-iteration accuracy via docs/plans/roadmap/27 Phase 4's pure computation applied to each
...
docs/plans/roadmap/10-format-versioning-and-chart-identity.md, arrangement id, section span, speed,
profile id from docs/plans/roadmap/27 Phase 1). Confirm the highway (docs/plans/roadmap/25) needs no change at
...
### Phase 6 — Practice UI and foot-controller input (gated: docs/plans/roadmap/20 Phase 0; 26 Phase 5)

Scope: entry points (Quick Play song entry → "Practice" per docs/plans/roadmap/26 Phase 7 list; in-song
pause menu → "Practice this section" per docs/plans/roadmap/27 Phase 6); section picker listing Phase 3
labels; speed control (range per open question 2); loop toggle; count-in click from the resource
pack (docs/plans/roadmap/20 Phase 2); on-highway section boundary markers and a per-iteration accuracy
readout. Bindable actions through docs/plans/roadmap/26 Phase 5's input layer with MIDI pedal bindings
via docs/plans/roadmap/24's IMidiTrigger: restart loop, speed down/up one step, toggle loop, exit
...
- Phase 3 extraction is a pure-code move; if the shared helper churns docs/plans/roadmap/27's landed
