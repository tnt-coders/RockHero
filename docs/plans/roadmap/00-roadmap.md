...
| docs/plans/roadmap/46-editor-keybinds.md | **Executing — Phases 0, 1, 5 complete 2026-07-20** | JUCE command-manager keybinds, config UI, diff persistence; Phase 4 rescoped (core trio non-rebindable — no injection seam, just the Ctrl+Shift+Z redo alias + predicate dedupe) | Registry spine + settled defaults landed 2026-07-20 (EditorView-owned ApplicationCommandManager, keybinds/ registry table, mapping set on MainWindow, menus via addCommandItem, editor-side predicates deleted, locked-table test; grammar keys stay in the grammar decoder with reserved chords — refinement recorded in the plan); remaining: Phase 2 persistence, Phase 3 dialog, rescoped Phase 4 |
