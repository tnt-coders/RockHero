...
| **G60-RULINGS** | docs/plans/roadmap/60-fhp-derivation.md §6 | User signs 60-Q1..Q5 (the five open rulings of `docs/plans/in-progress/fhp-derivation-algorithm.md`: the 11-rule algorithm, profiles, the fingering model, the derived `fhps` stream, the derived-default reversal) | 60 Phase 1 on Q1, Phase 2 on Q4, Phase 3 on Q3+Q5 (and on the span-marker template phases); the span-marker redesign's hand-coupling phase is downstream of 60 Phase 1 |
...
26e. docs/plans/roadmap/60-fhp-derivation.md — the corpus-measured FHP rebuild, in three beats against the span-marker redesign (user sequencing ruling 2026-09-05): **Phases 1–2 first** (the 11-rule floor tracker, a net code deletion, plus the derived `fhps` stream — basic derivation must stand alone under the zero-authored-input invariant: sane FHPs from a fresh import with no manually authored records); **Phase 3 merges into the span-marker plan's execution** (the manual authoring of FHP markers and span markers come together — the marker is one shared forced-statement boundary serving both machines, built once — plus the template coupling); **the hand coupling closes the sequence** (the span-marker plan's phase, clipping spans at this plan's FHP shifts, gated on the census seam-vs-shift agreement measure). The design record with the measured evidence is `docs/plans/in-progress/fhp-derivation-algorithm.md`.
...
| docs/plans/roadmap/60-fhp-derivation.md | Decision-gated (G60-RULINGS) | Corpus-measured 11-rule FHP floor tracker (88.03% vs ~70% shipped), derived-at-read stream over a three-record authored opinion layer, template/fingering coupling; prerequisite of the span-marker hand coupling | Not started — authored 2026-09-05 against `master @ 0d1b7009`; the design record `docs/plans/in-progress/fhp-derivation-algorithm.md` carries the measured evidence and the five open rulings |
