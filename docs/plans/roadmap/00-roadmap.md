...
| docs/plans/roadmap/55-pick-slide-notation.md | **Phases 1–2 complete 2026-08-03**; Phase 3 next behind the 55-Q1 head mockup pick | Pick slides as notes: `NoteAttack::PickSlide` with the traveled path in `slides` (pick coordinates, last offset == sustain, always traveling), in-memory technique overrides cleared by the document writer, gpif flags 64/128 import with the corpus-derived default path (down 17→3), full note-rule participation (occupancy, margin trims, deliberate holds), FHP transparency via the shared `rightHandOnset` predicate | Model twice revised on 2026-08-03 — borrowed-vocabulary experiment reverted, a separate `Chart::pick_slides` stream built then replaced by the note-carried design when the occupancy requirement landed; wavy unpitched path texture settled; head finalists (narrow whitened triangle vs X-shaped head) await mockup sheets; measure-3 dead hand-slides pinned out of scope by regression test |
