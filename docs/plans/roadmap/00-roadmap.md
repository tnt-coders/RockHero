...
- **25-Q6** capo display (raised 2026-08-09 at the matrix signing; **non-blocking by user call** — the data is already correct, absolute frets with 0 meaning the capo'd open string). Two halves: a graphical capo on the 3D highway at the capo fret (the neck below it is unplayable and currently looks ordinary), and a 2D tab-lane indicator that a capo exists. Design when either surface next gets attention.
