...
| **G60-RULINGS** | docs/plans/roadmap/60-fhp-derivation.md §6 | User signs 60-Q1..Q5 after a LAW-BY-LAW discussion session (the five open rulings of `docs/plans/in-progress/fhp-derivation-algorithm.md`); **deliberately TABLED 2026-09-05** until the task-list cleanup and the editor-functionality push (low-hanging fruit → keybinds + minimum editing → bend authoring) land | Plans 60 AND 61 entirely (60 Phase 1 on Q1, Phase 2 on Q4, Phase 3 + 61's authoring arc on Q3+Q5; the hand coupling closes the sequence) |
...
26e. **[TABLED 2026-09-05 — executes LAST, after the editor-functionality push]** docs/plans/roadmap/60-fhp-derivation.md + docs/plans/roadmap/61-span-marker-redesign.md — the corpus-measured FHP rebuild and the span-marker/template redesign, in three beats (user sequencing ruling 2026-09-05): **60 Phases 1–2 first** (the 11-rule floor tracker, a net code deletion, plus the derived `fhps` stream — basic derivation must stand alone under the zero-authored-input invariant: sane FHPs from a fresh import with no manually authored records and no fingering); **60 Phase 3 merges into 61's authoring arc** (FHP markers and span markers are one shared forced-statement boundary, built once, plus the template/dictionary coupling); **the hand coupling closes the sequence** (61's phase, clipping spans at 60's FHP shifts, gated on the census seam-vs-shift agreement measure). Gate G60-RULINGS requires a law-by-law discussion session before 60-Q1 signs. Before all of it, the current focus: task-list cleanup → low-hanging fruit → keybinds + minimum required editing functionality (author every aspect of a tab except FHPs/span markers) → bend authoring; an interim stopgap may materialize current stored FHPs + derived spans into hand-editable markers, ripped out when 60/61 land. The design record with the measured evidence is `docs/plans/in-progress/fhp-derivation-algorithm.md`.
...
### docs/plans/roadmap/60-fhp-derivation.md (gate G60-RULINGS)
...
| docs/plans/roadmap/60-fhp-derivation.md | **TABLED 2026-09-05** (G60-RULINGS; law-by-law session required) | Corpus-measured 11-rule FHP floor tracker (88.03% vs ~70% shipped), derived-at-read stream over a three-record authored opinion layer, template/fingering coupling; prerequisite of the span-marker hand coupling | Not started — parked until after the editor-functionality push; design record `docs/plans/in-progress/fhp-derivation-algorithm.md` carries the measured evidence, the tap-corrected regime, the sighting-gated width amendment, and the five open rulings. **2026-09-12**: the marker grammar reserves `Ctrl+P` (fret-hand position marker) and `Ctrl+H` (span marker) behind this gate, both inserting at the cursor and restating on selection; whether the two are one object or two is this gate's first ruling |
| docs/plans/roadmap/61-span-marker-redesign.md | **TABLED 2026-09-05** (rides G60-RULINGS) | Roadmap seat for the span-marker/template redesign: markers as forced statement boundaries, grip templates + dictionary, template editor, signed boundary gestures, the closing hand coupling | Not started — source of truth stays `docs/plans/todo/span-marker-redesign.md` + `chord-dictionary.md`; executes as one arc with plan 60 Phase 3 per the three-beat ruling. **2026-09-12**: the marker grammar reserves `Ctrl+H` (span marker) and `Ctrl+P` (position marker) behind G60-RULINGS, insert-at-cursor and restate-on-selection; the tabled `Shift+S` chord is withdrawn |
