...
26b. docs/plans/roadmap/55-pick-slide-notation.md Phases 3–4 (projection/tab marker + highway treatment; Phases 1–2 — `NoteAttack::PickSlide` model, format, rules, import — complete 2026-08-03). Phase 3's draw waits on the 55-Q1 head mockup pick; Phase 5's verb remainder folds into plan 40. Consumes plan 25's shipped drawers; any treatment colors enter item 26a's theme struct, never new file-scope constants.
26b. docs/plans/roadmap/56-head-atlas-mipmapping.md (mip chain for the GPU-sampled highway atlases, coverage-weighted, plus the cell-bleed decision) — user decision 2026-08-06 that this is wanted. Measured payoff: far-scale flicker drops from 4.34% to 0.08% mass variation and marks recover ~22% brightness the four-tap bilinear currently throws away, across all sixteen cells. **Sequence it WITH mark-authoring work, not before**: at the far edge an ordinary note gains 72.7% from correct filtering against a mark's 21.8%, so mips alone make every annotated note relatively less conspicuous at the horizon. Also settles the atlas edge convention (correct mip averaging requires the coverage-weighted weights the shipped cells already carry), which plan 54 Phase 3's texture contract then states rather than discovers.
...
| docs/plans/roadmap/40-chart-editing.md | **Executing** (elevated to co-track priority with the detection/scoring chain, user 2026-07-16) | Full chart authorability: selection, notes, techniques, curves, templates/shapes/FHPs/sections, bulk edit | Phases 1–3 complete + **Phase 4 substantially complete 2026-07-16 — charts are editable**: Alt+click insert, Delete, fret typing, sustain resize (Shift+Alt+arrows/Alt+wheel), Alt+arrows selection move — all single-undo-entry with 40-Q2-B truncation, persisted by write-on-save; keyboard grammar amended same day (plain keys never mutate; record in the interaction model doc); tail-drag parked as a watch item; remaining Phase 4 slice = pointer drag-move + Alt ghost preview; then Phase 5 (techniques); **Phase 11 (the New chart entry point) added 2026-08-08 as the last phase, gated on plan 41** — it was a standing to-do, now placed, and it is the only phase 41 blocks |
...
| docs/plans/roadmap/55-pick-slide-notation.md | **Phases 1–4 shipped (treatment sighted; shards built and deleted on sight 2026-08-05); Phase 5 verb layer shipped 2026-08-05** — remaining: the acceptance bundle (m20 sign, measure-3 local byte check, corpus smoke) and the head-texture revision as its own later pass | Pick slides as notes: `NoteAttack::PickSlide` with the traveled path in `slides` (pick coordinates, last offset == sustain, always traveling), in-memory technique overrides cleared by the document writer, gpif flags 64/128 import with the corpus-derived default path (down 17→3), full note-rule participation (occupancy, margin trims, deliberate holds), FHP transparency via the shared `rightHandOnset` predicate | Model twice revised on 2026-08-03 — borrowed-vocabulary experiment reverted, a separate `Chart::pick_slides` stream built then replaced by the note-carried design when the occupancy requirement landed; wavy unpitched path texture settled; head finalists (narrow whitened triangle vs X-shaped head) await mockup sheets; measure-3 dead hand-slides pinned out of scope by regression test |
