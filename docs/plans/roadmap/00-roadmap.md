...
| **G60-RULINGS** | docs/plans/roadmap/60-hand-markers.md §9 | User signs 60-Q1..Q5 after a LAW-BY-LAW discussion session (the five open rulings of `docs/plans/in-progress/fhp-derivation-algorithm.md`) plus the hand rulings 60-H2..H6 opened 2026-09-15 (60-H1, one object, is RULED); **deliberately TABLED 2026-09-05** until the task-list cleanup and the editor-functionality push (low-hanging fruit → keybinds + minimum editing → bend authoring) land | Plan 60 entirely (Phase 1 on Q1, Phase 2 on Q4, Phase 3 on H2+H3, Phase 5 on Q3+Q5+H4; Phase 6 closes on the census measure); Phase 0 is ungated |
...
The five derivation rulings below are unchanged from the former plan 60. The plan's §9 adds the
hand rulings of 2026-09-15: **60-H1** one object (RULED); **60-H2** the letter, id and record name
(R: `Ctrl+H` / `Ctrl+Shift+H`, `0x1516`; re-ask the key with the naming expert); **60-H3** the hand
row walks the derived hand events, authored markers among them (R); **60-H4** the open-stretch
window statement folds into the marker as an optional field (R, Phase 5); **60-H5** the pinned
chip is selectable like the ruler's pin (R); **60-H6** reveal on focus-row change (R). The rows'
decisions D6–D12 from the keyboard plan live there too (D6, D11, D12 RULED 2026-09-15).
