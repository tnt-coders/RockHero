...
| **GATE-A (detection contract)** | docs/plans/roadmap/22-note-detection.md Phase 1 | Detectability matrix + per-register latency budget co-signed with plan 24 | 22 Phases 2+; 24 Phase 2 (provisional-hit machine); 23 Phases 1/2/6 (event schema, bot, metrics) |
...
| docs/plans/roadmap/24-scoring-star-power-failure.md | Decision-gated (24-Q1..Q5) | Provisional-hit scoring machine, technique matrix, score record format, failure meter, MIDI star power | Not started |
...
against the clock, correct lane colors) witnessed on the same fresh build. Known defect logged at
sign-off, outside the milestone definition (one preloaded tone; no automation requirement): the
game does not apply tone parameter automation — see plan 21's status for the diagnosis; fix
queued as immediate follow-up work.
