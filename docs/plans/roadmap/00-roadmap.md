...
26b. docs/plans/roadmap/55-pick-slide-notation.md Phases 1–4 (pick-slide chart stream + import + projection/tab marker + highway treatment — executable now; 55-Q1 resolved 2026-08-03: separate `Chart::pick_slides` stream with waypoint paths, tap+tremolo-styled unpitched-slide rendering — 3D head numberless, 2D head shows the start position); only Phase 5 (editor gesture verbs) waits behind 55-Q2. Consumes plan 25's shipped drawers; any treatment colors enter item 26a's theme struct, never new file-scope constants.
...
### docs/plans/roadmap/55-pick-slide-notation.md (gate 55-Q2; 55-Q1 resolved)

- **55-Q1** format + visual treatment. **RESOLVED 2026-08-03 (user), after a full interrogation recorded in the plan's decision trail**: a pick slide is its own event kind — a separate `Chart::pick_slides` stream (`ChartPickSlide{position, anchor string, sustain, start fret, SlideWaypoint path}`; direction derived from the path, down-then-up chains supported like note slides), never a note field, so every technique exclusivity and FHP transparency are unrepresentable by construction. Rendering keeps the user's tap+tremolo composition in the unpitched-slide appearance: on the 3D highway a **numberless** head (the board's X axis places the start spatially; the missing digit differentiates from real tapped slides), in the 2D tab a head showing the **start neck position** under the white X full-mute marking (digit = neck position, X = unpitched; a superset of the print x + slanted-line convention) with per-leg target chips; tremolo-shimmer tail following the waypoint path; no text label. Evidence weighed and recorded in the plan: survey of the 4,100-arrangement local reference corpus (its chart format has NO standard composite — the tap+tremolo+slide triple covers ~31% of suspect notes; zero fret-0 unpitched slides), print convention research (X + wavy slanted line; "P.S." rejected as unaggressive; BandFuse never charted scrapes), and a rejected dedicated "noise ribbon" counter-proposal. The waypoint path makes the composition's slide component literally true, which is what settled it.
...
| docs/plans/roadmap/55-pick-slide-notation.md | Decision-gated (55-Q2 gesture verbs only — 55-Q1 resolved 2026-08-03); Phases 1–4 executable now | First-class right-hand pick-slide gestures: own chart stream (onset + span + neck-position waypoint path, direction derived, down-then-up chains), gpif flags 64/128 import with synthesized default path, FHP-transparent by construction, tap+tremolo-styled unpitched-slide rendering (numberless 3D head, start-position 2D head) | Not started — authored 2026-08-03 after the borrowed-vocabulary experiment was built and reverted on sight the same day; 55-Q1 then resolved through a local-reference-corpus survey (no standard composite exists in its chart format), notation research (P.S./wavy-line print convention; BandFuse negative), and a rejected dedicated-ribbon counter-proposal — the user kept the composition, made true by the waypoint path plus the per-surface head rule; measure-3-style dead hand-slides explicitly out of scope |
