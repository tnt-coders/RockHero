...
| **GATE-B (algorithm selection)** | docs/plans/roadmap/22-note-detection.md Phase 3 | Algorithm survey (dsp-guitar-detection-expert) sign-off incl. DSP dependency policy — **presented 2026-07-16** (selection memo appended to 22: in-repo NSDF+SuperFlux+Klapuri set on the vendored JUCE FFT; FFTW license-compatible but unwarranted; CQT physics-rejected with a recorded low-register revisit trigger; both carried verification items resolved), awaiting user sign-off | 22 Phase 6 (v1 detectors) |
...
| docs/plans/roadmap/22-note-detection.md | Decision-gated (GATE-B presented) | Detection contract + latency budget, dry tap, algorithm survey, pipeline, v1 detectors, tuner, tuning gate | Phases 1 + 3 complete; **GATE-A CLOSED 2026-07-16**; **GATE-B presented 2026-07-16** (selection memo appended, awaiting sign-off — Phase 6 blocked until signed; Phases 2/4/5 executable now) |
