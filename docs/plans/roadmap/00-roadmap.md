...
| **GATE-A (detection contract)** | docs/plans/roadmap/22-note-detection.md Phase 1 | Detectability matrix + per-register latency budget co-signed with plan 24 — **presented 2026-07-16** (dsp-expert-vetted, then amended same day via a three-pass max-effort review + user decisions: PitchStep legato onsets, PolyphonicSalience chord evidence, strum coalescing, evidence-gated lapse, overstrum streak-break behind a walk-back flag, committed-only display counters), awaiting user sign-off | 22 Phases 2+; 24 Phase 2 (provisional-hit machine); 23 Phases 1/2/6 (event schema, bot, metrics) |
| **GATE-B (algorithm selection)** | docs/plans/roadmap/22-note-detection.md Phase 3 | Algorithm survey (dsp-guitar-detection-expert) sign-off incl. DSP dependency policy | 22 Phase 6 (v1 detectors) |
...
- **22-Q1** v1 detectability tiers (GATE-A): adopt the amended matrix — Pop/Slap = Cosmetic, pinch-harmonic timbre = Cosmetic, palm-mute quality = Cosmetic with the note still scored, bend = Lenient per 24's trajectory rule, full-mute class check = Lenient with a ≥ 0.90 P/R promotion trigger (onset timing still Scored), chords = staged mono any-member rule promoting to ≥ 2 distinct non-octave members on measured evidence, legato = PitchStep-origin onsets? The matrix was expert-vetted twice and max-effort reviewed 2026-07-16 (see 22's Phase 1 vetting record). **R: adopt for v1**, revisit per-technique once plan 23 produces real precision/recall numbers.
...
| docs/plans/roadmap/22-note-detection.md | Decision-gated (GATE-A, GATE-B) | Detection contract + latency budget, dry tap, algorithm survey, pipeline, v1 detectors, tuner, tuning gate | Phase 1 executed 2026-07-16 (event types + expert-vetted contract); GATE-A presented, awaiting sign-off |
...
| docs/plans/roadmap/24-scoring-star-power-failure.md | Decision-gated (24-Q1..Q5) | Provisional-hit scoring machine, technique matrix, score record format, failure meter, MIDI star power | Phase 1 executed 2026-07-16 (ruleset, verdicts, window/ladder math); GATE-A presented, awaiting sign-off |
...
| docs/plans/roadmap/40-chart-editing.md | Ready | Full chart authorability: selection, notes, techniques, curves, templates/shapes/FHPs/sections, bulk edit | Not started |
