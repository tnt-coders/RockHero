...
### docs/plans/roadmap/62-kept-sustain-bound-per-song.md (no gate — build-session questions)
...
| docs/plans/roadmap/62-kept-sustain-bound-per-song.md | **Ready — ungated** (62-Q1..Q4 are build-session questions with recommendations) | The kept-sustain bound (shortest ring earning a drawn tail, both surfaces) becomes a per-song `song.json` value picked in the Song Information dialog; the shipped constant ("longer than an eighth", strict) stays the default for every song stating none | Not started — authored 2026-09-08 against `master @ 7bbe403b` after the inclusive comparison was sighted and reverted the same day and a tempo-derived bound was rejected for flipping inside songs near a threshold (both recorded in §11; the inclusive experiment tripled the corpus's standing tails, 52,990 → 167,819). Small: one constant, one reader, one derivation signature |
