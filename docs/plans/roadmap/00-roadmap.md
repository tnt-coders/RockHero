...
26e. docs/plans/roadmap/60-fhp-derivation.md — the corpus-measured FHP rebuild. **Phase 1 (the 11-rule floor tracker, a net code deletion) sequences BEFORE the span-marker redesign's hand-coupling phase**: that coupling clips spans at FHP shifts and is gated on this plan's accuracy (the census seam-vs-shift agreement measure is the readiness gate). Phase 2 (the derived `fhps` stream plus the two template-independent authored records; the sighting reels and external converter migrate in the #78 window) is independent of the span-marker work; Phase 3 (the template coupling) follows the span-marker plan's marker/template-editor phases. The design record with the measured evidence is `docs/plans/in-progress/fhp-derivation-algorithm.md`.
