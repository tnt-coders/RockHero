...
| docs/plans/roadmap/33-cross-platform-port.md | Ready (unscheduled — start at user direction) | Linux (Vulkan) + macOS (Metal) support for both executables under a binding minimize-platform-code principle (guards only where provably essential, one seam per concern, delete-before-add): generalization pass, shader profile table, backend enums, native handles, macOS bundling, CI hardening, real-GPU bring-up; editor 3D preview port separable (Phase 9) | Not started — authored 2026-07-18 from the three-audit investigation (guiding principle + platform-guard audit added same day); inventory re-verify required before execution |
