...
source for the game highway (docs/plans/roadmap/25-note-highway-3d.md), scoring timestamps
(docs/plans/roadmap/24-scoring-star-power-failure.md), the editor 3D preview
(docs/plans/roadmap/44-editor-3d-preview.md), and calibration sampling
...
- `rock-hero-game/` is a build-system skeleton (placeholder libs, 81-line window shell); the first
  ...
  docs/plans/roadmap/20-game-architecture-and-render-stack.md and docs/plans/roadmap/25-note-highway-3d.md, which
...
- docs/plans/roadmap/00-roadmap.md — execution ordering; this plan is scheduled early because milestone 0
  needs it for highway scrolling.
- docs/plans/roadmap/20-game-architecture-and-render-stack.md Phase 0b decides which thread the game
...
- docs/plans/roadmap/21-game-audio-engine-and-session.md — GameplaySession exposes `IPlaybackClock` to
  gameplay; publishes a non-1.0 `playback_rate` once speed control exists.
- docs/plans/roadmap/24-scoring-star-power-failure.md — scoring timestamps and the coherent-snapshot
  revisit (open question 1 below).
- docs/plans/roadmap/25-note-highway-3d.md — highway render consumes the extrapolator each frame.
- docs/plans/roadmap/44-editor-3d-preview.md — editor preview consumes the same extrapolator (this is why
  it lives in common, per constraint (a)).
- docs/plans/roadmap/13-audio-device-settings-and-calibration.md — calibration capture samples the clock.
- docs/plans/roadmap/28-practice-mode.md — requires that plan 12/21 interfaces carry a playback speed
...
- docs/plans/todo/smooth-scroll-follow-evaluation.md — the smooth-scroll follow evaluation is an owned,
...
12. The snapshot gains two fields beyond the absorbed doc's `{position, playing}`:
    ...
    docs/plans/roadmap/21-game-audio-engine-and-session.md publishes otherwise). Capture stamps turn
    ...
    rate field is docs/plans/roadmap/28-practice-mode.md's non-negotiable day-one plumbing.
...
1. **Snapshot time domain for scoring correlation.** Options: (A) timeline nanoseconds only (v1
   ...
   when docs/plans/roadmap/24-scoring-star-power-failure.md defines its provisional-hit contract. (B) Add
...
2. **Editor cursor migration / `ITransport::position()` retirement.** Options: (A) migrate the
   ...
   correct there) and revisit when docs/plans/roadmap/44-editor-3d-preview.md and the smooth-scroll
   decision (docs/plans/todo/smooth-scroll-follow-evaluation.md, awaiting your call) create real
...
- **Snap rules**: snap to target on (1) first call, (2) playing-state transition (start/resume),
  ...
  (backward jumps are legitimate, from docs/plans/roadmap/28-practice-mode.md), and pathological stalls.
...
**Scope.** No code. Record in this plan (and mirror in docs/plans/roadmap/00-roadmap.md status) the
...
- `ITransport::position()` remains the editor's message-thread path pending open question 2; its
  retirement is re-evaluated in docs/plans/roadmap/44-editor-3d-preview.md.
...
- **Escalation guard**: do not attempt the audio-graph tap node or any submodule patch inside this
  ...
  insufficient, open the escalation as its own registered follow-up in docs/plans/roadmap/00-roadmap.md
