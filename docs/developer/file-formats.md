...
than pointing at it. A PITCHED keyframe never sits on a later onset of its own string for the same
reason; the release may park there, since it names where the hand leaves toward and never a
landing. linkNext identity-fragmentation is rejected for the same reason.
