...
| `tuning.capo` | int | opt | (`0`) Capo fret, `0..12`. Frets are **absolute**, and `0` means the open string capo'd or not — so the capo never appears as a fret number, and frets `1..capo` are invalid on notes, postures, waypoint fret statements, and fret-hand positions. |
...
| `notes[].vibrato` / `.tremolo` | bool | opt | Written only when true. `vibrato` is the shake at the ONSET — the vibrato channel's opening statement, which holds until the first waypoint that states the channel again, so a note shaking end to end simply states it here and never restates it. `tremolo` means UNMEASURED noise picking (as fast as possible, no real timing) — the charting standard spells out measured fast repetition as discrete notes instead. |
...
| `notes[].waypoints[]` | object[] | opt | The note's ONE interval payload: `{offset: <fraction> req, fret opt, bend opt, vibrato opt}`, ascending, each offset strictly inside `(0, sustain]`. A waypoint fixes a MOMENT and carries any SUBSET of the channels that can change while a string sounds, so a glide target, a bend value and a vibrato change authored at one instant are one record rather than three coincident copies of an offset (`docs/plans/todo/unified-waypoint-model.md`). Each channel reads independently: `fret` interpolates between fret-STATING waypoints (equal frets = a hold, different = travel) and a fret-less waypoint is pass-through for position; `bend` interpolates from the onset value and holds flat past the last statement; `vibrato` holds from each statement until the next. Absence of a channel is a MEANING, so a stated `0` bend (a release) and a stated `false` vibrato (a shake ending) are both written out. A waypoint stating NO channel is refused. A stated `fret` never sits on a later onset of the string (a shift glide ends the minimum sustain distance before its re-picked landing) and lies strictly before the ring's end when `slideOut` is present. The removed `slides[]` key is **refused** rather than read. |
