...
| `notes[].vibrato` / `.tremolo` | bool | opt | Written only when true. `tremolo` means UNMEASURED noise picking (as fast as possible, no real timing) — the charting standard spells out measured fast repetition as discrete notes instead. |
