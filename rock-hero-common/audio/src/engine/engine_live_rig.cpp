    ...
    const std::string& tone_document_ref)
{
    ...
    // THE ONE OWNERSHIP RULE, and the reason it lives here rather than at any call site: while a
    // schedule is baked the audio thread already drives the branch gains from the curves, and has
...
{
    m_load_op->missing_plugin_names.push_back(display_name + " (" + tone_document_ref + ")");
    ++m_load_op->next_plugin;
    ++m_load_op->completed_plugins;
    reportLiveRigLoadProgress(
        m_load_op->request,
        m_load_op->completed_plugins,
