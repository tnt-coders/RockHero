    ...
    const common::core::Arrangement& arrangement)
{
    ...
    //    invalidates the proxy hash and schedules a fresh offline render + restartPlayback,
    //    stalling the slider. Proxy-off routes through WaveNodeRealTime's elastique reader, which
    //    stretches the original source in realtime and responds to speed changes immediately.
