...
{
    CHECK(std::is_eq(clampGain(Gain{-100.0}).db <=> minimumGainDb()));
    CHECK(std::is_eq(clampGain(Gain{-25.0}).db <=> minimumGainDb()));
}

// Verifies that clampGain clamps values above the maximum.
TEST_CASE("clampGain clamps above maximum", "[audio][gain]")
{
    CHECK(std::is_eq(clampGain(Gain{100.0}).db <=> maximumGainDb()));
    CHECK(std::is_eq(clampGain(Gain{25.0}).db <=> maximumGainDb()));
}
