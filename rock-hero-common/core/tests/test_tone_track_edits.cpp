...
[[nodiscard]] ToneRegion makeRegion(
    std::string id, GridPosition start, GridPosition end,
...
TEST_CASE("createToneRegion splits the region containing the position", "[core][tone-edits]")
{
    ...
    track.regions.push_back(makeRegion(
        "a", GridPosition{.measure = 1, .beat = 1}, GridPosition{.measure = 5, .beat = 1}));

    const auto result =
        createToneRegion(track, GridPosition{.measure = 3, .beat = 1}, "b", "tones/y/tone.json");

    REQUIRE(result.has_value());
    REQUIRE(track.regions.size() == 2);
    CHECK(track.regions[0].id == "a"); // the earlier tone runs up to the new marker
    CHECK(track.regions[0].start == GridPosition{.measure = 1, .beat = 1});
    CHECK(track.regions[0].end == GridPosition{.measure = 3, .beat = 1});
    CHECK(track.regions[1].id == "b"); // the new region begins at the marker
    CHECK(track.regions[1].start == GridPosition{.measure = 3, .beat = 1});
    CHECK(track.regions[1].end == GridPosition{.measure = 5, .beat = 1});
...
TEST_CASE("createToneRegion splits the correct region among several", "[core][tone-edits]")
{
    ...
    track.regions = {
        makeRegion(
            "a", GridPosition{.measure = 1, .beat = 1}, GridPosition{.measure = 3, .beat = 1}),
        makeRegion(
            "b", GridPosition{.measure = 3, .beat = 1}, GridPosition{.measure = 5, .beat = 1}),
    };

    const auto result =
        createToneRegion(track, GridPosition{.measure = 4, .beat = 1}, "c", "tones/y/tone.json");

    REQUIRE(result.has_value());
    REQUIRE(track.regions.size() == 3);
    CHECK(track.regions[1].id == "b");
    CHECK(track.regions[1].end == GridPosition{.measure = 4, .beat = 1});
    CHECK(track.regions[2].id == "c");
    CHECK(track.regions[2].start == GridPosition{.measure = 4, .beat = 1});
...
TEST_CASE("createToneRegion rejects a position not inside any region", "[core][tone-edits]")
{
    ...
    for (const GridPosition position :
         {GridPosition{.measure = 1, .beat = 1},
          GridPosition{.measure = 3, .beat = 1},
          GridPosition{.measure = 5, .beat = 1}})
    {
    ...
    "deleteToneRegion extends the previous region over the removed span", "[core][tone-edits]")
{
    ...
    track.regions = {
        ...
        makeRegion(
            "b", GridPosition{.measure = 3, .beat = 1}, GridPosition{.measure = 5, .beat = 1}),
        makeRegion(
            "c", GridPosition{.measure = 5, .beat = 1}, GridPosition{.measure = 7, .beat = 1}),
    };
    ...
    CHECK(track.regions[0].end == GridPosition{.measure = 5, .beat = 1}); // extended over b
...
TEST_CASE("deleteToneRegion on the first region extends the next backward", "[core][tone-edits]")
{
    ...
    CHECK(track.regions[0].start == GridPosition{.measure = 1, .beat = 1}); // extended back
